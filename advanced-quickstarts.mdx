---
title: "Advanced Quickstarts"
description: "Build real SEC API workflows beyond your first request, with practical end-to-end recipes instead of toy examples."
---

# Advanced Quickstarts

<Info>
Use these once the first request works. Each quickstart is a real workflow, not a toy example.
</Info>

## Pick the next job

<CardGroup cols={2}>
  <Card title="Compare 13F positions" icon="users" href="/api-reference/owners">
    Compare manager positions and quarter-over-quarter changes.
  </Card>
  <Card title="Compare executive compensation" icon="wallet-cards" href="/api-reference/compensation">
    Compare pay packages and compensation mix across time.
  </Card>
  <Card title="Search offerings activity" icon="briefcase-business" href="/api-reference/offerings">
    Pull new S-1 and 424B4 activity without staying in EDGAR all day.
  </Card>
  <Card title="Open billing or portal" icon="credit-card" href="/auth-and-pricing">
    Move from evaluation to a paid plan or inspect the billing portal.
  </Card>
</CardGroup>

### Compare 13F positions

```bash
curl -X POST \
  -H "content-type: application/json" \
  -H "x-api-key: $SECAPI_API_KEY" \
  https://api.secapi.ai/v1/owners/13f/compare \
  -d '{"cik":"0001067983","limit":25}'
```

### Compare executive compensation

```bash
curl -X POST \
  -H "content-type: application/json" \
  -H "x-api-key: $SECAPI_API_KEY" \
  https://api.secapi.ai/v1/compensation/compare \
  -d '{"ticker":"NVDA","limit":10}'
```

### Search offerings activity

```bash
curl -H "x-api-key: $SECAPI_API_KEY" \
  "https://api.secapi.ai/v1/offerings?forms=S-1,424B4&limit=10"
```

### Create a billing or portal session

```bash
curl -X POST \
  -H "content-type: application/json" \
  -H "x-api-key: $SECAPI_API_KEY" \
  https://api.secapi.ai/v1/billing/checkout \
  -d '{"planKey":"personal","successUrl":"https://secapi.ai/onboarding?checkout=success","cancelUrl":"https://secapi.ai/onboarding?checkout=cancel"}'
```

<Prompt>
Take the next SEC API workflow after the first request. Choose one of: compare 13F positions, compare executive compensation, search offerings activity, or open the billing flow. Implement it with the exact endpoint, preserve requestId and freshness metadata, and summarize the result in markdown.
</Prompt>
