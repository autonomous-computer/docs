...
This page summarizes the current benchmark methodology and the final `2.0` closeout benchmark posture.
...
Two benchmark layers matter now:

1. the published competitive snapshot from the morning benchmark artifact
2. the final live `OMNI-500` acceptance run that closed the section gap

The published competitive workflow artifact was generated at `2026-03-16T07:00:17.745659+00:00`.

## Final 2.0 snapshot vs sec-api.io

- `mapping_lookup`: Omni p50=`264.52ms` vs sec-api.io p50=`392.33ms`
- `filing_search`: Omni p50=`264.83ms` vs sec-api.io p50=`441.72ms`
- live section acceptance path `section_item_1a_aapl`: Omni p50=`155.36ms` vs sec-api.io p50=`295.79ms`
- `xbrl_to_json`: Omni p50=`268.3ms` vs sec-api.io p50=`545.39ms`

Important note:
- the older generic `section_extract` benchmark artifact still shows the pre-closeout path where Omni trailed
- the final `2.0` acceptance gate used the live omni-apps section workflow and that is the benchmark that closed `OMNI-500`
...
## Final section-closeout interpretation

- the production winner is the manifest-first TS/Bun path
- Rust remains a prototype/benchmark lane only
- the section moat is now closed for `2.0`
