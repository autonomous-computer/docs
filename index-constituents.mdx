---
title: "Index Constituents | SEC API"
description: "Retrieve index rosters with company names, tickers, rank order, and provenance, mapped into a canonical entity graph where coverage exists."
---
...
The following indices are available to principals with `admin:operator` scope. They are derived from our market-reference read model and are approximate — not official committee selections.
...
Admin-only indices appear in `/v1/market/indices` but return a 400 error from `/v1/market/indices/constituents` unless the caller has admin scope.
...
- SEC API can prove strong private mapping coverage for `SPX`, `NDX`, `DJIA`, and `RUT` without implying that every one of those rosters is public
...
- `public_source_review_required` means SEC API tracks the official source but has not yet cleared public constituent export
- `licensed_index_not_supported` means SEC API will not expose the constituent roster until explicit licensing is in place

## Sync posture

- `NDX` is the only public constituent roster today, and its export is generated from SEC API's checked-in source definition
- inventory exports are generated from SEC API's checked-in index-source definitions
