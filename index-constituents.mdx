...
- internal entity mapping now covers `NDX`, `SPX`, `DJIA`, and `RUT` through the canonical remote API even though public constituent export remains narrower

## Inventory-only indices

- `SPX` is tracked, but constituent export remains blocked pending index licensing
- `DJIA` is tracked, but constituent export remains blocked pending index licensing
- `RUT` is tracked for internal mapping audit work, but public constituent export still requires documented shareability review
