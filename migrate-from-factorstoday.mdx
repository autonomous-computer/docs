...
This guide maps FactorsToday-style factor-history, valuation, portfolio, hedge, optimizer, dashboard, and bulk-download workflows to SEC API equivalents. Use it as a practical cutover guide for moving factor-ranked workflows, portfolio analytics, and agent tools onto SEC API's REST, SDK, CLI, and MCP surfaces.

<Info>
Start with the user workflow: history tables, 1D through MAX windows, extreme moves, valuation views, attribution, hedging, optimization, and bulk exports. Then wire the SEC API endpoint that returns the same job with compact responses, request diagnostics, and trust metadata.
</Info>
