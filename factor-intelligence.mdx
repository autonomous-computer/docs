...
- launch-ready factors are organized into market, style, sector, and industry categories
...
- launch factors target a `2010-01-01` history floor; shorter-history or beta factors are labeled as such and should not be used for 2010+ coverage claims
...
- single-factor daily history with exact 1D through MAX summary windows
...
- live intraday factor return snapshots derived from the latest benchmark proxy inputs
...
- consolidated one-call view combining intraday returns, regime context, rotation signals, and current factor state
...
| `POST` | `/v1/models/factor-analysis` | ad hoc model factor analysis |
