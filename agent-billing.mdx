...
```text
Check billing posture first. Quote the expensive step before repeating it. If the org is in payg_pending_card, stop and ask the administrator to activate PAYG before continuing. If the server requires approval or reports a hard cap, stop instead of retrying blindly.
```
