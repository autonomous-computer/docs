...
description: "See how much you'll save with SEC API's pay-as-you-go pricing vs sec-api.io's fixed monthly plans"
---

SEC API is the only SEC data API with true pay-as-you-go pricing. No monthly minimum. Pay only for what you use.

## Side-by-Side Comparison

### Light Usage (Agent running ~200 queries/month)

| | sec-api.io | SEC API PAYG | SEC API Personal |
|---|---|---|---|
...
| | sec-api.io | SEC API PAYG | SEC API Team |
|---|---|---|---|
...
<CardGroup cols={3}>
  <Card title="Starter grant (150 calls)" icon="gift">
...
|---|---|---|---|
| Entity resolution | $0.01 | $0.008 | $0.0065 |
...
| Webhook or stream delivery | $0.001 | $0.0008 | $0.00065 |
| Monitor email notification | $0.005 | $0.004 | $0.00325 |

## What sec-api.io Doesn't Offer

- **No PAYG tier.** Minimum $49/month even for 1 query.
- **No no-card starter grant.** 100 lifetime calls vs our 150-call one-time grant (with an optional self-serve reset every 30 days).
- **No per-call billing.** Fixed monthly regardless of usage.
- **No budget controls.** No spending caps, no threshold alerts.
- **No intelligence bundles.** 8+ API calls to assemble what SEC API returns in 1.
