...
SEC API ships enterprise-grade infrastructure from the first request. Audit logging, entitlement reporting, key rotation, and multi-seat organizations are built into the platform today. OAuth/SSO via WorkOS is on the enterprise roadmap for 2026 Q3+.
...
Hosted MCP clients use WorkOS OAuth discovery through `api.secapi.ai`. OAuth discovery endpoints are published at `/.well-known/oauth-protected-resource` and `/.well-known/oauth-authorization-server`. This is the only customer-visible WorkOS integration today.

### OAuth/SSO — enterprise roadmap

OAuth/SSO via [WorkOS](https://workos.com) is planned for the enterprise tier in 2026 Q3+. When it ships, it will cover SAML and OIDC federation with Okta, Azure AD, Google Workspace, and other SAML 2.0 / OIDC identity providers for customers who require corporate-IdP-backed human login. Contact the SEC API team to register interest and coordinate rollout timing.

## Audit Logs

Every API request generates a traceable event chain. SEC API provides multiple customer-safe APIs for inspecting activity after the fact.
