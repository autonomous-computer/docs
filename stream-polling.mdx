...
description: "Use cursor-based stream polling for deterministic SEC API event replay."
---
...
```bash
curl "$SEC_API_URL/v1/stream_subscriptions/str_123/events?limit=25" \
...
```bash
curl "$SEC_API_URL/v1/stream_subscriptions/str_123/events?cursor=evt_987" \
  -H "Authorization: Bearer $SEC_API_KEY"
```
