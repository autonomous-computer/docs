...
description: "Stream SEC filings the moment they hit EDGAR with WebSocket and webhook APIs. Sub-minute latency, filterable by form type and ticker."
---

# Real-Time SEC Filing Stream API

Get SEC filings delivered to your application the moment they appear on EDGAR. SEC API monitors the EDGAR full-text feed continuously and pushes new filings through WebSocket connections and webhook callbacks with sub-minute latency.
...
<CardGroup cols={2}>
  ...
  <Card title="Webhook callbacks" icon="bell">
    Register an endpoint and SEC API will POST structured filing data to your server with automatic retries and HMAC signing.
  </Card>
...
## Why SEC API for real-time filings
...
<Card title="Get your API key" icon="key" href="/getting-started">
  Create an API key and start streaming SEC filings in under 60 seconds.
</Card>
