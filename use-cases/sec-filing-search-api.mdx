...
description: "Search SEC filings by company, form type, date range, and content. Full-text and semantic search across millions of EDGAR documents."
---
...
curl -H "x-api-key: $SEC_API_KEY" \
  "https://api.secapi.ai/v1/search/semantic?q=artificial+intelligence+risks+and+regulation&form=10-K&limit=5"
```
...
| Section extraction | `/v1/filings/{accession_number}/sections/{section_key}` | Pull specific sections from any filing |
...
```bash
curl -H "x-api-key: $SEC_API_KEY" \
  "https://api.secapi.ai/v1/search/semantic?q=merger+acquisition+definitive+agreement&form=8-K&limit=10"
```
...
| API access | REST + SDKs | Limited | closed workstation | Proprietary |
...
<Card title="Get your API key" icon="key" href="/getting-started">
  Create an API key and start searching filings in under 60 seconds.
</Card>
