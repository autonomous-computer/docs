...
description: "Access structured SEC enforcement action data via API. Search by violation type, respondent, date range, and penalty amount. Track fraud cases and regulatory trends."
---

# SEC Enforcement Actions API

The SEC brings hundreds of enforcement actions each year against individuals and companies for securities fraud, insider trading, accounting violations, and other regulatory breaches. SEC API structures this data and exposes it through a searchable API.
...
```bash
curl -H "x-api-key: $SEC_API_KEY" \
  "https://api.secapi.ai/v1/events/enforcement?query=insider+trading&limit=10"
```
...
|-----------|-------------|---------|
| `query` | Search by violation, respondent, or release text | `insider trading` |
| `limit` | Results per page | `20` |

## Why SEC API for enforcement data

| Feature | SEC API | SEC.gov | LexisNexis | Bloomberg Law |
|---------|-----------|---------|------------|---------------|
