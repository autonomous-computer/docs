...
description: "Extract individual sections from SEC 10-K filings via API. Get Item 1 (Business), 1A (Risk Factors), 7 (MD&A), 7A (Market Risk), and 8 (Financial Statements) as clean text."
---
...
```bash
curl -H "x-api-key: $SEC_API_KEY" \
  "https://api.secapi.ai/v1/filings/latest/sections/item_7?ticker=MSFT&form=10-K"
```
...
<Card title="Get your API key" icon="key" href="/getting-started">
  Create an API key and start extracting 10-K sections in under 60 seconds.
</Card>
