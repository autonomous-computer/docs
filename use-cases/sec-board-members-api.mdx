...
description: "Access structured board of directors data extracted from SEC proxy statements (DEF 14A) via API. Track board composition, independence, committees, and tenure."
---
...
<CardGroup cols={2}>
  ...
  <Card title="Committee membership" icon="users-round">
...
curl -H "x-api-key: $SEC_API_KEY" \
  "https://api.secapi.ai/v1/board?ticker=TSLA"
```
...
Filter board records by company and filing context:

```bash
curl -H "x-api-key: $SEC_API_KEY" \
  "https://api.secapi.ai/v1/board?ticker=TSLA&limit=10"
```
...
|----------|-------------|
| `GET /v1/board` | Board composition and director records |
...
<Card title="Get your API key" icon="key" href="/getting-started">
  Create an API key and start analyzing board composition in under 60 seconds.
</Card>

- [Executive Compensation API](/use-cases/executive-compensation-api)
