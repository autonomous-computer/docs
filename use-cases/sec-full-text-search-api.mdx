...
description: "Search the full text of SEC filings with keyword and AI-powered semantic search. Find risk factors, M&A disclosures, and specific clauses across millions of documents."
---

# SEC Full Text Search API

Search inside the text of every SEC filing — not just metadata. SEC API indexes the full text of filings on EDGAR and provides both traditional keyword search and AI-powered semantic search that understands meaning.
...
curl -H "x-api-key: $SEC_API_KEY" \
  "https://api.secapi.ai/v1/search/semantic?q=risks+related+to+AI+regulation+and+compliance&form=10-K&limit=5"
```
...
```bash
curl -H "x-api-key: $SEC_API_KEY" \
  "https://api.secapi.ai/v1/search/semantic?q=material+weakness+in+internal+controls&ticker=AAPL&form=10-K&limit=10"
```
...
Compare how companies in the same sector describe their competitive position, pricing strategy, or market positioning.
...
|----------|-------------|
| `GET /v1/search/semantic` | AI-powered semantic search |
...
| Feature | SEC API | EDGAR EFTS | Bloomberg | S&P Capital IQ |
|---------|-----------|-----------|-------------------|----------------|
