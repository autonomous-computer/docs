...
<Steps>
  ...
  <Step title="Set the API key and base URL">
    Export `SECAPI_API_KEY` and default to `https://api.secapi.ai` unless you are testing a local or staging environment.
  </Step>
...
client = SecApiClient(api_key=os.environ["SECAPI_API_KEY"], retry=False, telemetry=False, timeout=10)
```
