...
The observability methods require an operator-scoped key (`admin:operator`).

```python
operator_client = SecApiClient(api_key=os.environ["SECAPI_API_KEY"])
artifact = client.create_artifact({
    "ticker": "AAPL",
    "form": "10-K",
    "sectionKey": "item_1a",
...
observability = operator_client.observability()
```
