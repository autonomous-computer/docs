...
`SecApiError` keeps the raw `payload` and also exposes `code`, `message`, and `request_id` convenience properties. Log those fields with the failing workflow so support can trace the exact request without asking users to parse JSON by hand.
