...
Generated from the SEC API contract and prompt asset registry.

<Note>New here? Start at [secapi.ai/agents](https://secapi.ai/agents) for a one-paste MCP install across Claude Code, Cursor, Codex, and Windsurf, a drop-in [rules file](https://secapi.ai/agents/AGENTS.md), and installable skills (`npx skills add secapi-ai/secapi-skills`).</Note>

<Note>For persona-tuned prompts (50 prompts × 5 customer segments — law firms, investment managers, sophisticated investors, insurance, PR firms), see the [agent prompt library](/agents/prompt-library).</Note>
...
- Resolve entities (ticker, CIK, CUSIP, ISIN, FIGI) before querying via GET /v1/entities/resolve, and pass the resolved CIK downstream — never guess a CIK.
- Prefer structured SEC API endpoints and MCP tools over raw filing text when possible.
- When a number comes from a filing, cite the source accession number and the filing page, and keep extracted filing facts separate from your interpretation.
...
- Preserve requestId, traceparent, SECAPI-Plan-Key, SECAPI-Billing-State, SECAPI-Free-Grant-Remaining, provenance, freshness, and materialization metadata on every downstream artifact.
...
- `/v1/statements/all` — Return the balance sheet, income statement, and cash flow statement as one normalized bundle. Resolves us-gaap (10-K/10-Q) and IFRS foreign-private-issuer (20-F/6-K) filers automatically.
- `/v1/entities/resolve` — Resolve an entity by ticker, symbol, CIK, FIGI-family identifier, ISIN, CUSIP, name, or query with confidence and match-basis metadata
- `/v1/entities` — Search canonical SEC entities across issuers, managers, insiders, and funds
- `/v1/entities/edgar` — Search SEC EDGAR entity records by name, ticker, CIK, or identifier
