---
title: "SEC API Dilution Score"
---

# SEC API Dilution Score

A single 0-100 derived metric expressing near-term dilution risk per issuer. Built by composing five factor signals from the SEC API dilution dataset (filed offering events, outstanding warrants and convertibles, latest cash position, share-float trajectory, Nasdaq compliance status). Returns six categorical sub-ratings, a `regsho` flag, and an optional rolling-history timeseries materialized daily.
...
- degraded reasons are reported explicitly so consumers can reason about gaps instead of treating the score as full-confidence
