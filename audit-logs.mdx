...
- `summary.support` with a first-read finding, next actions, latest usage
  status, delivery failure count, and missing artifact IDs

Use this as the first investigation step when a customer or teammate provides a `Request-Id`.
...
```bash
curl "$SECAPI_BASE_URL/v1/delivery/events?kind=event&type=artifact.created&limit=25" \
  -H "x-api-key: $SECAPI_API_KEY"
```
