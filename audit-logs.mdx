...
SEC API records every API interaction as a traceable event. Audit surfaces are available through request diagnostics, event listing and export, delivery summaries, and operator observability exports. Request, event, and delivery endpoints require org-scoped authentication via API key or bearer token; observability endpoints require `admin:operator`.
...
Use this as the first investigation step when a customer or support team provides a `Request-Id`.
...
```bash
curl "$SECAPI_BASE_URL/v1/events?kind=event&type=artifact.created&limit=25" \
  -H "x-api-key: $SECAPI_API_KEY"
```
...
curl "$SECAPI_BASE_URL/v1/events/export?since=2026-04-09T00:00:00Z&format=ndjson&limit=1000" \
  -H "x-api-key: $SECAPI_API_KEY" \
...
| `/v1/observability` | GET | Operator-only observability configuration |
| `/v1/observability/export` | GET | Operator-only bundled observability export |
