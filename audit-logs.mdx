...
SEC API records every API interaction as a traceable event. Audit surfaces are available through request diagnostics, delivery event listing/export, delivery summaries, and operator observability exports. Request, event, and delivery endpoints require org-scoped authentication via API key or bearer token; observability endpoints require `admin:operator`.
