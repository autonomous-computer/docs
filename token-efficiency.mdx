...
SECAPI_RESPONSE_SIZE=$(curl -s "https://api.secapi.ai/v1/entities/resolve?ticker=AAPL" \
  -H "x-api-key: $SECAPI_KEY" | wc -c)

SEC_API_SIZE=$(curl -s "https://api.sec-api.io/mapping/ticker/AAPL" \
  -H "Authorization: $SEC_API_KEY" | wc -c)

echo "SEC API: $SECAPI_RESPONSE_SIZE bytes, sec-api.io: $SEC_API_SIZE bytes"
```
