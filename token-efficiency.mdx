...
2. **Compact mode.** Use `?view=compact` when your workflow only needs the fields required for the next step.
...
SECAPI_SIZE=$(curl -s "https://api.secapi.ai/v1/entities/resolve?ticker=AAPL" \
  -H "x-api-key: $SECAPI_API_KEY" | wc -c)

SEC_API_IO_SIZE=$(curl -s "https://api.sec-api.io/mapping/ticker/AAPL" \
  -H "Authorization: $SECAPI_API_KEY" | wc -c)

echo "SEC API: $SECAPI_SIZE bytes, sec-api.io: $SEC_API_IO_SIZE bytes"
```
