...
<Steps>
  <Step title="Set the API key">
    Export `SEC_API_KEY` first. Every other surface will fail in ways that look less clear if auth is not in place.
  </Step>
...
The public CLI package name is `@secapi/cli`, with the `secapi` binary. Use it for quick REST calls from a terminal when the package is available in your registry.
...
secapi intelligence company --ticker AAPL --json
```

Prefer direct REST commands for launch-critical examples. Use hosted MCP for agent workflows that need tool discovery.
