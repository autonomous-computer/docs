---
title: "Libraries & SDKs"
---

# Libraries & SDKs

<Info>
Pick the integration that matches your runtime. Then prove it with one real entity lookup, one filing workflow, one structured result, and one traceable source-backed read before you move to production.
</Info>

## Choose Your Integration

<CardGroup cols={2}>
  <Card title="JavaScript SDK" icon="file-code-2" href="/javascript-sdk">
    Best for TypeScript services, apps, scripts, and product integrations.
  </Card>
  <Card title="Python SDK" icon="file-code" href="/python-sdk">
    Best for research notebooks, data workflows, and backend services.
  </Card>
  <Card title="Go REST Guide" icon="binary" href="/go-sdk">
    Best for lightweight backend services using direct HTTP calls.
  </Card>
  <Card title="Rust REST Guide" icon="cpu" href="/rust-sdk">
    Best for async and performance-sensitive services using direct HTTP calls.
  </Card>
  <Card title="Hosted MCP" icon="bot" href="/mcp-workflows">
    Best for coding agents and MCP-aware runtimes that need grounded investor workflows.
  </Card>
  <Card title="SEC API CLI" icon="terminal" href="#sec-api-cli">
    Best when you want quick REST calls from the terminal without wiring up an SDK.
  </Card>
  <Card title="REST API" icon="globe" href="/api-reference">
    Best when you want exact HTTP control or your own runtime wrapper.
  </Card>
</CardGroup>

## Recommended sequence

<Steps>
  <Step title="Set the API key">
    Export `SEC_API_KEY` first. Authentication failures are easier to debug before adding SDK, CLI, or MCP layers.
  </Step>
  <Step title="Verify one entity lookup">
    Confirm your integration can resolve a company or filer you already know, ideally through the identifier type you expect to use in production.
  </Step>
  <Step title="Verify one filing workflow and one semantic intelligence workflow">
    Pull a filing or statements bundle, then prove one-call market/macro/factor/intelligence output before you call the integration complete.
  </Step>
  <Step title="Preserve the metadata">
    Keep `requestId`, provenance, freshness, materialization metadata, and trace references in your logs or downstream artifacts.
  </Step>
</Steps>

## Give this prompt to your agent

<Prompt description="Copy this SEC API prompt for your agent.">
Install the right SEC API integration for this project. Prefer the native SDK if the runtime matches, otherwise use the REST API. Then verify one entity lookup, one filing workflow, one semantic investor workflow such as `intelligence.query`, and one trace lookup while preserving requestId, provenance, freshness, materialization metadata, and trace references.
</Prompt>

## SEC API CLI

The public CLI package name is `@secapi/cli`, with the `secapi` binary. Use it for quick REST calls from a terminal when the package is available in your registry.

### Installation

```bash
npm install -g @secapi/cli
```

### Usage

```bash
export SEC_API_KEY=your_key_here
secapi entities resolve --ticker AAPL --json
secapi filings search --ticker AAPL --form 10-K --limit 3 --json
secapi intelligence company --ticker AAPL --json
```

Prefer direct REST commands for production examples. Use hosted MCP for agent workflows that need tool discovery.

## Read next

<CardGroup cols={3}>
  <Card title="Auth and pricing" icon="key-round" href="/auth-and-pricing">
    The auth model, version header, and plan state.
  </Card>
  <Card title="MCP workflows" icon="plug-zap" href="/mcp-workflows">
    Hosted MCP setup and agent-facing usage patterns.
  </Card>
  <Card title="API Reference" icon="square-terminal" href="/api-reference">
    Exact endpoint behavior when you need wire-level precision.
  </Card>
</CardGroup>
