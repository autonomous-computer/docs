...
<Info>
SEC API gives insurance and risk teams programmatic access to financial health indicators, restatement and auditor change alerts, executive compensation data, and enforcement history -- all through a single API with webhook-driven monitoring and full provenance.
</Info>
...
| `/v1/webhook_endpoints` | `POST` | Register webhook endpoints for event-driven monitoring |
...
Register an org-level webhook endpoint to receive monitor-match alerts:

```bash
curl -X POST \
  -H "content-type: application/json" \
  -H "x-api-key: $SECAPI_API_KEY" \
  -d '{"destinationUrl":"https://your-system.example.com/webhooks/sec","description":"Insurance risk monitor","subscribedEventTypes":["monitor.match"]}' \
...
Webhook deliveries include the same provenance and `requestId` metadata as direct API calls. Use `/v1/delivery/events/export` to audit delivery history.
...
| Skill | What it does |
|-------|-------------|
| `sections.search` | Search filing sections for underwriting screening |
| `companies.financials` | Pull normalized issuer financial statements |
| `comp.list` | Retrieve executive compensation records |
| `filings.search` | Find filings by company, form type, and date range |
...
<CardGroup cols={2}>
  <Card title="Webhook-driven monitoring" icon="radio" href="/webhook-stream-workflows">
    Create org-level webhook endpoints for monitor matches; define entity and form filters on monitors. No polling infrastructure required.
  </Card>
  <Card title="Commercial plan" icon="building" href="/plans-and-pricing">
