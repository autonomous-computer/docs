...
`SecApiError` normalizes the common SEC API and JSON-RPC error shapes. Use [Status](/status), `code`, `requestId`, `message`, and `body` in structured logs so support can connect customer reports to the exact failed request.
