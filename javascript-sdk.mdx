...
<Steps>
  <Step title="Install the package">
    Use the public npm package name once it is available in your package registry.
  </Step>
  <Step title="Set the API key">
    Export `SEC_API_KEY` in the runtime where the code will run.
  </Step>
...
If the npm package is not visible in your registry yet, use the REST example below. The SDK mirrors the same endpoints and `x-api-key` authentication.
...
```ts
const headers = { "x-api-key": process.env.SEC_API_KEY! }
...
When the public SDK package is available, the intended npm package name is `@secapi/sdk`:
...
```ts
import { SecApiClient, SecApiError } from "@secapi/sdk"
