...
SEC API provides a structured macro surface covering indicator time series, release history, forward estimates, regime classification, and curated high-signal country packs. All endpoints return `requestId` and `traceparent` for observability.
...
| `limit` | positive integer | -- | Maximum observations returned |
...
| `limit` | positive integer | -- | Maximum releases returned |
...
|-----------|------|---------|-------------|
| `country` | string | `US` | ISO country code |
| `days` | positive integer | -- | Look-ahead window in days |
...
| `country` | string | `US` | ISO country code |
| `indicator` | string | -- | Filter to a specific indicator key (alias: `indicator_key`) |
| `horizons` | positive integer | -- | Number of forward horizons to return |
