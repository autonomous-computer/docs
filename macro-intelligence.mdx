...
`/v1/macro/status` reads validation artifacts, not live vendor probes. Use it when an agent needs to decide whether macro output is ready, degraded, stale, or unsupported.

The compact response includes country status, source posture, alerts, freshness age, and methodology. Use `response_mode=verbose` or `view=full` only when you need indicator-level rows.
