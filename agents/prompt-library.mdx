...
Each prompt names the exact MCP tools it expects to call, the output artifact (memo, table, JSON export, scorecard), and any known caveats. Every v1 entry is structurally validated against the live MCP server contract on every PR.
...
<Note>10 additional prompts are queued for delivery alongside the dilution tooling expansion and AAER MCP wrap. They are visible via `secapi agents prompts list --include-v2` and labelled `coming soon` on each persona page.</Note>
