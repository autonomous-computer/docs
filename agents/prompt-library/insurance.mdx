...
<Tip>For a renewal book, batch no more than 50 tickers per company call. Retain the accession number, filing date, section, and minimal supporting excerpt with every alert.</Tip>
...
These prompts create underwriting and claims-research packets; they do not score risk or set coverage terms. Keep the cited filing record with each alert, flag missing evidence, and stop on auth, validation, billing, quota, or capability errors.
