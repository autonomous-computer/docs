...
description: "Filing-led prompts for forensic accounting, ownership review, and thesis stress-testing."
---

Use these prompts to pressure-test a thesis against filings, XBRL facts, reported ownership, and dated market context. They expose assumptions and source gaps; they do not establish fraud, manipulation, mispricing, or an investment conclusion.
...
These prompts help test a thesis; they do not prove fraud, mispricing, or intent. Keep source records beside each inference, call out missing evidence, and stop when credentials, schema, billing, quota, or tool capability fail.

## Working rules

- `filings.search` and `sections.search` do not apply an MCP date window. Filter candidate records using their returned `filingDate` before drawing a time-bounded result.
- State every researcher-created calculation, its inputs, its period alignment, and what the source does not disclose.
- Treat a search result, factor output, or ownership report as a lead or context until the underlying source supports the claim.
...
For every thesis-critical finding, preserve the source filing, fiscal period, accession number, section or fact tag, and calculation inputs. A missing record, an anomalous output, or a reported holder change is a research question, not proof or a trading instruction.
