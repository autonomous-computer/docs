...
description: "Source-backed briefing prompts for public-company communications, investor relations, and issue response."
---

Use these prompts to build a factual source pack before a client briefing, release review, investor-relations update, or issue-response discussion. They capture what the issuer disclosed; they do not determine materiality, predict market reaction, or replace counsel and issuer review.
...
These prompts assemble factual briefing material; they do not decide materiality or messaging. Keep source filings next to client-facing claims, separate verified facts from open questions, and stop on auth, validation, billing, quota, or capability errors.

## Working rules

- `filings.search` and `sections.search` do not apply an MCP date window. Filter returned `filingDate` values before you assemble the briefing.
- Preserve the issuer's terms. Do not turn a disclosed event, vote, or transaction into a motive, sentiment signal, or governance judgment.
- Keep verified facts, open questions, and proposed messaging in separate sections.
...
Keep the form, filing date, accession number, section, and source URL with every brief. Re-check the cited disclosure against the final client-facing language before publication.
