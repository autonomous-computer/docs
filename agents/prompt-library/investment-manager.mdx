---
title: "Investment manager — Agent Prompt Library"
...
### 4. Run a macro-regime-aware factor alignment review
...
2. `companies.income_statements` — Trailing four quarters
3. `sections.get` — Latest MD&A from 10-Q
4. `filings.search` — Recent earnings-related 8-K disclosures
...
```
For META, pull the latest named-executive compensation disclosures and the prior year's compensation, compare year-over-year base + equity + total, then overlay the latest SEC-derived income statement and ratio context. Output an alignment scorecard (0-10) with bullets on (a) pay-for-performance context, (b) equity grant timing relative to reported financial trends, (c) any outlier comp items vs peers.
```
...
4. `companies.ratios` — SEC-derived financial ratio context
