...
Stress-test a portfolio against supported macro and factor scenarios. Returns projected drawdown, recovery timeline, and per-holding impact when scenario inputs are available.
...
| `category` | string | no | Optional scenario category filter. |
| `scenarioKey` | string | no | Specific supported scenario key such as `us_recession`, `higher_for_longer`, or `china_growth_scare`. |
...
curl -X POST \
  -H "x-api-key: $SECAPI_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"holdings":[{"symbol":"AAPL","weight":0.5},{"symbol":"TLT","weight":0.5}],"scenarioKey":"us_recession"}' \
...
|-------|------|----------|-------------|
| `holdings` | array | yes | Array of `{ symbol, weight }` objects. |
| `objective` | string | no | Optimization objective. One of `factor_neutral`, `min_drawdown`, `regime_aware`. Defaults to `factor_neutral`. |
| `country` | string | no | ISO 3166-1 alpha-2 country code. Defaults to `US`. |
