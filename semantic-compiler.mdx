...
**Status:** Available as a deterministic planning utility. Route-level integration is tracked separately.
...
1. **Planner utility** — `QueryPlan` type, `compileQuery()` function, tests, and docs.
2. **Route integration** — wire `compileQuery` into `/v1/intelligence/query` as an alternative to the existing classifier.
