...
Do not infer a route's bill from its name. The current authenticated `/v1/situations*` request class is `situations_lookup`; delivery is separately accounted for as `webhook_delivery`, `stream_event`, or `email_delivery` when a delivery is actually attempted. Preserve delivery IDs and source filing provenance, verify inbound signatures over the raw body, and replay only failed deliveries when the receiving system is ready.
...
Before a batch or an agent loop, retrieve `GET /v1/limits` and `GET /v1/billing/rates`. For every request, log the response meter and billing headers rather than hard-coding a plan assumption. The current Special Situations read class is `situations_lookup`; delivery work has separate `webhook_delivery`, `email_delivery`, and `stream_event` classes. Rates and entitlements are organization-specific.
