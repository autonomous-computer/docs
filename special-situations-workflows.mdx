...
Creating, reading, or changing a watch reports the `monitor_management` control-plane meter. Successful organization-level webhook delivery is metered as `situations_webhook_delivery`, stream fanout delivery as `stream_event`, and direct email delivery as `email_delivery`. Preserve delivery IDs and source filing provenance, verify inbound signatures over the raw body, and replay only failed deliveries when the receiving system is ready.

## MCP and reusable Skills

The hosted MCP surface currently exposes `situations.list`, `situations.search`
(an alias of `situations.list`), `situations.by_form`, `situations.get`,
`situations.filings`, `situations.summary`, `situations.feed`,
`situations.calendar`, `situations.stats`, `situations.performance`,
`situations.export`, `situations.watch`, `situations.issues`,
`situations.issue`, and `situations.underwrite`. Tool
...
Before a batch or an agent loop, retrieve `GET /v1/limits` and `GET /v1/billing/rates`. For every request, log the response meter and billing headers rather than hard-coding a plan assumption. The important Special Situations meter classes are `situations_lookup`, `situations_feed_read`, `situations_export`, `situations_underwriting_pack`, `monitor_management`, `email_delivery`, `situations_webhook_delivery`, and `stream_event`.

Proprietary Omni Apps fundamentals, estimates, valuation, and symbol coverage are internal-only and excluded from the public Special Situations API, documentation, SDKs, CLI, MCP, and pricing.
