...
Situation watchlist create, read, and delete operations are paid Special Situations API calls. Log the returned `SECAPI-Meter-Class` response header instead of hard-coding a meter assumption. Poll-only watchlists do not send alerts by themselves; poll `GET /v1/monitors/{monitor_id}/matches` to read matches for the saved monitor ID. Delivery activation may require bearer/org authentication, an additional delivery entitlement, and delivery-specific metering. Preserve delivery IDs and source filing provenance, verify inbound signatures over the raw body, and replay only failed deliveries when the receiving system is ready.
