...
<Prompt>
Set up Omni Datastream auth with an org-scoped API key. Verify health, readiness, billing, rates, and limits first. Then run one real issuer workflow while preserving `requestId`, `traceparent`, `Omni-Plan-Key`, `Omni-Billing-State`, `Omni-Free-Grant-Remaining`, provenance, and freshness metadata.
</Prompt>
