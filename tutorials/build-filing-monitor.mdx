...
Open the signed-in [SEC API dashboard](https://secapi.ai/app/webhooks), add your public HTTPS destination, and subscribe it to `monitor.match`. Save the signing secret when it is shown. Endpoint creation and secret rotation are human-authenticated organization actions; an API key cannot perform them.

SEC API sends webhook deliveries with `x-secapi-signature: t=<unix_seconds>,v1=<hex_hmac>` and `x-secapi-signature-timestamp`. Verify the signature against the exact raw request body before parsing JSON.
