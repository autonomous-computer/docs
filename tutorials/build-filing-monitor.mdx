---
title: "Build a Filing Monitor with Webhooks"
description: "Set up real-time monitors for SEC filings and receive webhook notifications when new 8-K events are filed. Includes curl, Python, and JavaScript examples."
---

# Build a Filing Monitor with Webhooks

Stop polling for new filings. Instead, register a webhook endpoint and create a monitor rule so the API pushes events to you the moment a filing hits EDGAR.

## Prerequisites

- An SEC API key (set as `SECAPI_API_KEY`)
- A publicly accessible HTTPS endpoint to receive webhooks (use [webhook.site](https://webhook.site) for testing)
- (Optional) Python 3.8+ or Node.js 18+ for SDK examples

## Step 1 — Register a webhook endpoint

Tell the API where to send events by creating a webhook endpoint with `/v1/webhook_endpoints`.

### curl

```bash
curl -X POST \
  -H "x-api-key: $SECAPI_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "destinationUrl": "https://your-server.com/webhooks/filings",
    "description": "Filing monitor webhook",
    "subscribedEventTypes": ["monitor.match"]
  }' \
  "https://api.secapi.ai/v1/webhook_endpoints"
```

### Python

```python
from secapi_client import SecApiClient

client = SecApiClient(api_key="your-api-key")

endpoint = client.create_webhook_endpoint(
    destination_url="https://your-server.com/webhooks/filings",
    description="Filing monitor webhook",
    subscribed_event_types=["monitor.match"],
)

print(f"Endpoint ID: {endpoint['id']}")
print(f"Signing secret: {endpoint['signingSecret']}")
```

### JavaScript

```ts
import { SecApiClient } from "@secapi/sdk-js";

const client = new SecApiClient({
  apiKey: process.env.SECAPI_API_KEY!,
});

const endpoint = await client.createWebhookEndpoint({
  destinationUrl: "https://your-server.com/webhooks/filings",
  description: "Filing monitor webhook",
  subscribedEventTypes: ["monitor.match"],
});

console.log(`Endpoint ID: ${endpoint.id}`);
console.log(`Signing secret: ${endpoint.signingSecret}`);
```

### Expected output

```json
{
  "id": "we_abc123",
  "destinationUrl": "https://your-server.com/webhooks/filings",
  "signingSecret": "whsec_...",
  "subscribedEventTypes": ["monitor.match"]
}
```

Save the `signingSecret` securely. You will use it to verify incoming webhook payloads.

## Step 2 — Create a filing monitor

A monitor defines what filings trigger a webhook delivery. Use `/v1/monitors` to watch for specific form types and companies.

### curl

```bash
curl -X POST \
  -H "x-api-key: $SECAPI_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "FAANG 8-K Monitor",
    "query": "latest FAANG 8-K filings",
    "filters": {
      "forms": ["8-K", "8-K/A"],
      "tickers": ["AAPL", "AMZN", "GOOG", "META", "NFLX"]
    }
  }' \
  "https://api.secapi.ai/v1/monitors"
```

### Python

```python
monitor = client.create_monitor(
    name="FAANG 8-K Monitor",
    query="latest FAANG 8-K filings",
    filters={
        "forms": ["8-K", "8-K/A"],
        "tickers": ["AAPL", "AMZN", "GOOG", "META", "NFLX"],
    },
)

print(f"Monitor ID: {monitor['id']}")
print(f"Active: {monitor['isActive']}")
```

### JavaScript

```ts
const monitor = await client.createMonitor({
  name: "FAANG 8-K Monitor",
  query: "latest FAANG 8-K filings",
  filters: {
    forms: ["8-K", "8-K/A"],
    tickers: ["AAPL", "AMZN", "GOOG", "META", "NFLX"],
  },
});

console.log(`Monitor ID: ${monitor.id}`);
console.log(`Active: ${monitor.isActive}`);
```

### Expected output

```json
{
  "id": "mon_xyz789",
  "name": "FAANG 8-K Monitor",
  "query": "latest FAANG 8-K filings",
  "filters": {
    "forms": ["8-K", "8-K/A"],
    "tickers": ["AAPL", "AMZN", "GOOG", "META", "NFLX"]
  },
  "isActive": true
}
```

## Step 3 — Handle incoming webhooks

When a matching filing is published, SEC API emits a `monitor.match` event to the org-level webhook endpoint you subscribed in Step 1. Here is what the payload looks like and how to verify it.

### Webhook payload

```json
{
  "eventType": "monitor.match",
  "timestamp": "2024-12-15T14:30:00Z",
  "data": {
    "monitorId": "mon_xyz789",
    "matches": [
      {
        "accessionNumber": "0000320193-24-000095",
        "form": "8-K",
        "ticker": "AAPL",
        "companyName": "Apple Inc",
        "filingDate": "2024-12-15"
      }
    ]
  }
}
```

### Python handler (Flask)

```python
import hmac
import hashlib
from flask import Flask, request, jsonify

app = Flask(__name__)
SIGNING_SECRET = "whsec_..."

@app.route("/webhooks/filings", methods=["POST"])
def handle_webhook():
    # Verify signature
    signature = request.headers.get("X-SecAPI-Signature", "")
    parts = dict(part.split("=", 1) for part in signature.split(",") if "=" in part)
    timestamp = parts.get("t", "")
    provided = parts.get("v1", "")
    expected = hmac.new(
        SIGNING_SECRET.encode(),
        f"{timestamp}.".encode() + request.data,
        hashlib.sha256,
    ).hexdigest()

    if not hmac.compare_digest(provided, expected):
        return jsonify({"error": "Invalid signature"}), 401

    payload = request.json
    for match in payload["data"].get("matches", []):
        print(f"New {match['form']} from {match['ticker']}")
        print(f"Accession: {match['accessionNumber']}")

    return jsonify({"received": True}), 200
```

### JavaScript handler (Express)

```ts
import express from "express";
import crypto from "crypto";

const app = express();
const SIGNING_SECRET = "whsec_...";

app.post("/webhooks/filings", express.raw({ type: "application/json" }), (req, res) => {
  const signature = req.headers["x-secapi-signature"] as string;
  const parts = new Map(signature.split(",").map((part) => part.split("=", 2) as [string, string]));
  const timestamp = parts.get("t") ?? "";
  const provided = parts.get("v1") ?? "";
  const expected = crypto
    .createHmac("sha256", SIGNING_SECRET)
    .update(`${timestamp}.${req.body.toString("utf8")}`)
    .digest("hex");

  if (provided.length !== expected.length || !crypto.timingSafeEqual(Buffer.from(provided), Buffer.from(expected))) {
    return res.status(401).json({ error: "Invalid signature" });
  }

  const payload = JSON.parse(req.body.toString());
  for (const match of payload.data.matches ?? []) {
    console.log(`New ${match.form} from ${match.ticker}`);
    console.log(`Accession: ${match.accessionNumber}`);
  }

  res.json({ received: true });
});

app.listen(3000);
```

## Step 4 — List and manage monitors

### curl

```bash
# List all active monitors
curl -H "x-api-key: $SECAPI_API_KEY" \
  "https://api.secapi.ai/v1/monitors"

# Check recent matches
curl -H "x-api-key: $SECAPI_API_KEY" \
  "https://api.secapi.ai/v1/monitors/mon_xyz789/matches?limit=10"

# Deactivate a monitor
curl -X DELETE \
  -H "x-api-key: $SECAPI_API_KEY" \
  "https://api.secapi.ai/v1/monitors/mon_xyz789"
```

## Next steps

- **Monitor more form types**: Add `10-K`, `10-Q`, `13F-HR`, or any other form type to your monitors.
- **Route by event type**: Create different webhook endpoints for different teams (e.g., compliance gets enforcement actions, research gets earnings filings).
- **Audit delivery**: Use the [Webhook Delivery Audit](/webhook-delivery-audit) page to inspect delivery logs and retry failures.

See the [Webhook Stream Workflows](/webhook-stream-workflows) guide for advanced patterns.
