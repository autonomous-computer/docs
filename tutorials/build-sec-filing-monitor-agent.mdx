...
description: "Build an autonomous agent that monitors SEC 8-K filings daily using Claude Code and the SEC API CLI, then sends Slack alerts for your watchlist."
---

# Build an SEC Filing Monitor Agent

Build an autonomous agent that checks for new 8-K filings every day and sends Slack notifications when companies on your watchlist file material events. This tutorial uses Claude Code skills and the SEC API CLI to create a fully automated filing monitor you can run on a schedule.

## What you will build

- A Claude Code skill that runs on a cron schedule
- Daily checks against the SEC API for new 8-K filings
...
## Step 1 — Install the SEC API CLI
...
Create `skill.js` with the core logic that queries the SEC API for recent 8-K filings.
...
2. Query the SEC API for recent 8-K and 8-K/A filings for each ticker
...
0 23 * * 1-5 cd /path/to/sec-filing-monitor-agent && SEC_API_KEY="your-api-key" node skill.js >> /var/log/sec-monitor.log 2>&1
```
