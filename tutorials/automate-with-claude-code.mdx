...
description: "Use Claude Code cron skills to run recurring SEC analysis workflows with SEC API. Build a weekly 13F comparison report that runs every Friday evening."
---
...
## Endpoints Used
- GET /v1/owners/13f?cik={cik}&date={date}
...
2. Fetch the previous quarter 13F for comparison:
   ```bash
curl -H "x-api-key: $SEC_API_KEY" \
     "https://api.secapi.ai/v1/owners/13f?cik=0001067983&limit=1&offset=1"
   ```
...
| API key not available in cron | Make sure `SEC_API_KEY` is exported in your shell profile, not just the current session. |
