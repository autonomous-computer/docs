...
description: "Build a Node.js agent that generates pre-earnings research briefs using SEC API intelligence bundles, factor decomposition, and insider activity data."
---

Before a company reports earnings, analysts need a quick-hit research brief covering the company's fundamentals, factor exposures, and recent insider activity. This tutorial builds a Node.js agent that pulls data from multiple SEC API endpoints and assembles a pre-earnings research brief you can share with your team.
...
```bash
npm install @secapi/sdk-js dotenv
```
...
Create `index.js` with functions that pull data from each SEC API endpoint.
...
async function fetchCompanyIntel(ticker) {
  const intel = await client.intelligenceCompany({ ticker });
  return intel;
}

/**
 * Fetch the earnings preview intelligence bundle — pre-built earnings
 * context including filing-derived performance, recent guidance, and key topics.
 */
async function fetchEarningsPreview(ticker) {
  const preview = await client.intelligenceEarningsPreview({ ticker });
  return preview;
}
...
async function fetchFactorDecomposition(ticker) {
  const factors = await client.factorDecomposition({ symbol: ticker });
  return factors;
}

/**
 * Fetch recent Form 4 transaction rows for the report. The transaction code
 * and filing link remain available for a reviewer to interpret the activity.
 */
async function fetchInsiderActivity(ticker) {
  const insiders = await client.insiders({
    ticker,
    forms: "4",
    limit: 20,
  });
...
async function fetchEarningsDate(ticker) {
  ...
  const calendar = await client.marketEarningsCalendar({
    ticker,
...
function generateReport(ticker, intel, preview, factors, insiders, earnings) {
  ...
  if (insiders?.data && insiders.data.length > 0) {
    ...
    for (const txn of insiders.data.slice(0, 15)) {
      const date = txn.transactionDate || txn.filingDate || "N/A";
      const name = txn.ownerName || "Unknown";
      const title = txn.ownerTitle || "";
      const type = txn.transactionCode || txn.transactionDirection || "N/A";
      const shares = txn.transactionShares?.toLocaleString() ?? "N/A";
      const price = txn.transactionPrice == null ? "N/A" : `$${txn.transactionPrice.toFixed(2)}`;
      lines.push(`| ${date} | ${name} | ${title} | ${type} | ${shares} | ${price} |`);
    }
    lines.push("");

    // Reported Form 4 code counts. These are not an investment signal.
    const buys = insiders.data.filter(
      (t) => t.transactionCode === "P"
    ).length;
    const sells = insiders.data.filter(
      (t) => t.transactionCode === "S"
    ).length;
    lines.push(
      `**Reported Form 4 code counts (last 20 rows):** P=${buys}, S=${sells}`
    );
    lines.push("");
  } else {
  ...
  lines.push("---");
  lines.push(
    "*Data sourced from SEC EDGAR via SEC API. This is not investment advice.*"
  );
...
async function batchPreview() {
  ...
  const tickers = (calendar.data || []).map((e) => e.symbol).slice(0, 10);
...
Use the SEC API intelligence query endpoint to add custom research questions to the brief.

```javascript
async function askQuestion(ticker, question) {
  const result = await client.intelligenceQuery({
    query: question,
    entities: [ticker],
    responseMode: "compact_json_and_md",
  });
  return result?.answer || result?.markdown || result;
}
