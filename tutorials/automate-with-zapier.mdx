...
description: "Create a Zapier Zap that tracks insider trades via SEC API webhooks and logs them to Google Sheets with Slack notifications. Step-by-step configuration included."
---

# SEC Data Automation with Zapier

Zapier connects thousands of apps with no-code automations called Zaps. In this tutorial you will build a Zap that receives SEC API webhook events for new insider trades (Form 4 filings), logs each trade to a Google Sheets spreadsheet, and sends a Slack notification.
...
5. Copy this URL --- you will register it with SEC API next.
...
1. Name your Zap (e.g., "SEC API Insider Trade Tracker").
...
- **Enrich with SEC API data**: Add a **Webhooks by Zapier > GET** step to call `/v1/insiders?ticker={ticker}` and pull additional context before logging.
