...
description: "Build an n8n workflow that receives SEC API webhook events for new SEC filings and routes alerts to Slack. Includes importable workflow JSON."
---

n8n is an open-source workflow automation tool you can self-host or run in the cloud. In this tutorial you will wire an n8n webhook node to the SEC API so that every time an 8-K filing is published for a watchlist of companies, n8n extracts the key data and posts a summary to a Slack channel.
...
EDGAR filing published
        |
        v
SEC API monitor (8-K filings, watchlist tickers)
        |
        v
...
4. Set **Path** to `secapi-filings` (or any path you prefer).
...
```
https://your-n8n.example.com/webhook-test/secapi-filings
```

Once the workflow is active, use the **Production URL** instead:

```
https://your-n8n.example.com/webhook/secapi-filings
```

## Step 2 --- Register the webhook with SEC API

Point the SEC API monitor at your n8n webhook URL.
...
curl -X POST \
  ...
  -d '{
    "url": "https://your-n8n.example.com/webhook/secapi-filings",
...
Add a second **Function** node between the Webhook node and the extraction node to verify the SEC API signing secret. If verification fails, the workflow stops.

```javascript
const crypto = require("crypto");

const SIGNING_SECRET = "whsec_..."; // Replace with your signing secret
const signature = $input.first().json.headers["x-secapi-signature"];
...
```json
{
  "name": "SEC API SEC Filing Alerts",
  "nodes": [
    {
      "parameters": {
        "httpMethod": "POST",
...
| 401 or signature mismatch | Verify you are using the correct `signing_secret` from the SEC API webhook endpoint. |
...
| Duplicate alerts | SEC API retries failed deliveries. Make the workflow idempotent by checking `accession_number` against a dedup store (e.g., n8n's built-in static data). |
...
- **Enrich with intelligence**: After receiving a filing event, call the SEC API `/v1/intelligence/query` endpoint to get an AI summary before posting.
