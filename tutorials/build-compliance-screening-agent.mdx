...
An organization administrator can create the webhook endpoint with `POST /v1/webhook_endpoints` and the monitor with `POST /v1/monitors`. Those public organization routes use `Authorization: Bearer $SECAPI_BEARER_TOKEN`; an `x-api-key` is not sufficient for the mutation. The signed-in [SEC API dashboard](https://secapi.ai/app/webhooks) is an optional way to configure or inspect the same resources. Use a narrow monitor name, query, form filter, and ticker set. Store the signing secret returned when the endpoint is created in `SECAPI_WEBHOOK_SIGNING_SECRET`.

You need Python 3.9+, Flask, a public HTTPS receiver, the signing secret, a bearer token to create or change the organization resources through the API, and an API key only if you later query monitor matches.

## 2. Verify the raw delivery
...
## 3. Make the review record durable
...
Replace `print` with one transactional write that stores the event id, source URL, and a pending-review status. Send a `webhook.test` from the dashboard or `POST /v1/webhook_endpoints/{webhook_id}/test` to exercise signature verification; it proves receiver handling, not that a monitor query has found a filing. See [webhook delivery audit](/webhook-delivery-audit) for delivery investigation.
