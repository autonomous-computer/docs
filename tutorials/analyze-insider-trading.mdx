...
description: "Track insider buys and sells using SEC Form 4 data from the SEC API. Identify patterns in executive transactions. Includes curl, Python, and JavaScript examples."
---
...
For the product workflow version of this tutorial, see the [insider trading monitor](https://secapi.ai/workflows/insider-trading-monitor), [ownership feature overview](https://secapi.ai/features/ownership), and [Form 4 glossary page](https://secapi.ai/glossary/form-4).
...
transactions = client.insiders(ticker="AAPL", limit=10)
...
const transactions = await client.insiders({
  ticker: "AAPL",
  limit: 10,
});
...
```ts
const buys = await client.insiders({
  ticker: "JPM",
...
- **Use the product workflow**: The [insider trading monitor](https://secapi.ai/workflows/insider-trading-monitor) shows how to package this into an agent-ready monitoring workflow.
- **Review Form 4 context**: The [Form 4 glossary page](https://secapi.ai/glossary/form-4) covers transaction codes, deadline caveats, and signal traps.
