...
description: "Learn how to pull structured revenue data from SEC 10-K filings using the SEC API. Includes curl, Python, and JavaScript examples."
---
...
const filing = await client.latestFiling({
  ticker: "AAPL",
  form: "10-K",
});
...
```python
statements = client.all_statements(
    ticker="AAPL",
    period="annual",
    limit=1,
)
...
```bash
curl -H "x-api-key: $SECAPI_API_KEY" \
  "https://api.secapi.ai/v1/facts?ticker=AAPL&tag=Revenues&period=annual&limit=4"
```
...
for fact in facts["data"]:
    print(f"{fact['periodEnd']}: ${fact['value']:,.0f}")
```
