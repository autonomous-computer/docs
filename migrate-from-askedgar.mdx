...
$0.018/KB and a typical ~6 KB response. SEC API quotas verified against
`infra/stripe/pricing-plans.json`. The 150 included calls are a one-time
grant (with an optional self-serve reset every 30 days) per
`apps/docs/content/billing-faq.md`.
...
- `askedgar_url` is preserved on every SEC API dilution row, so you can keep a back-link to the AskEdgar Copilot UI if your operators want to dual-source during cutover.
