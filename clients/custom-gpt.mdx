...
description: "Connect SEC API to a Custom GPT — native MCP via ChatGPT Apps Developer Mode, or an OpenAPI Action fallback for any tier."
---

Two install paths depending on your ChatGPT plan. Path 1 gives a Custom GPT (or any chat) the full MCP toolset via ChatGPT Apps. Path 2 wraps the REST API as an OpenAPI Action so a Custom GPT on any tier can still call SEC API.

## Prerequisites

- SEC API key (free — 150 calls, one-time). Create an account at [secapi.ai/signup](https://secapi.ai/signup).
...
6. Save. Enable the connector in any chat; the SEC API MCP tools are now available.
...
2. **Schema**: paste the URL `https://docs.secapi.ai/openapi/sec-api.v1.json` and import.
...
- **"Schema import failed" (Path 2)** — verify the schema URL returns JSON with `curl https://docs.secapi.ai/openapi/sec-api.v1.json | head`.
