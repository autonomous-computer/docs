...
- SEC API key (free — 250 calls/month). Sign up at [secapi.ai](https://secapi.ai). Set `SEC_API_KEY`.
...
const client = new MultiServerMCPClient({
  "sec-api": {
    transport: "streamable_http",
    url: "https://api.secapi.ai/mcp",
    headers: { "x-api-key": process.env.SEC_API_KEY! },
  },
});
...
- **`401 Unauthorized`** — `x-api-key` is missing or wrong. Sanity check: `curl https://api.secapi.ai/v1/limits -H "x-api-key: $SEC_API_KEY"`.
...
- **Streaming notifications missing** — if your adapter version does not surface MCP progress events, tool calls can still complete while long-running progress updates are omitted.
