...
- SEC API key (free — 250 calls/month). Sign up at [secapi.ai](https://secapi.ai). Export `SEC_API_KEY` in your shell.
...
Open the Cascade chat, click the MCP icon, and hit **Refresh**. If the config is loaded and the key is valid, the SEC API MCP tools appear in the tool list.
...
- **`401 Unauthorized`** — environment variable is unset or unreadable from Windsurf's process. Restart Windsurf after `export SEC_API_KEY=…`, or replace `${env:SEC_API_KEY}` with the literal key for a quick test.
