---
title: "Install SEC API in Windsurf"
description: "Connect SEC API to Windsurf's Cascade agent by editing one JSON config file — full MCP support with environment-variable auth interpolation."
---
...
- SEC API key (free — 150 calls, one-time). Create an account at [secapi.ai/signup](https://secapi.ai/signup). Export `SECAPI_API_KEY` in your shell.
...
Open the Cascade chat, click the MCP icon, and hit **Refresh** — Windsurf does not auto-watch the config file. The SEC API MCP tools should appear in the tool list.
...
- **`401 Unauthorized`** — environment variable is unset or unreadable from Windsurf's process. Start Windsurf from the same terminal after `export SECAPI_API_KEY=…`, or configure your desktop/IDE launcher to provide that environment variable.
