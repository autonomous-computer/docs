...
description: "Add SEC API as a tool spec in LlamaIndex using MCP over streamable HTTP."
---
...
async def main():
    http = httpx.AsyncClient(headers={"x-api-key": os.environ["SEC_API_KEY"]})
...
- **`401 Unauthorized`** — the `httpx.AsyncClient` headers didn't reach the server. Verify the env var is set in the same shell that runs Python: `echo $SEC_API_KEY`.
