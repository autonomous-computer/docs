...
<Warning>
**Preliminary — pending external methodology review**. Numbers on
...
All numbers on this page come from our open benchmark harness:

- A SEC API benchmark suite (entity resolve, filing search, section extract, XBRL → JSON)
- An equivalent edgartools benchmark suite run under the same conditions
- A 4-way competitive scorecard aggregator
- A reviewable methodology doc

The methodology is published so every number here is reproducible and auditable. If you spot a methodology issue, email support@secapi.ai.
