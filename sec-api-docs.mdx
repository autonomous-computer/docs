---
title: "SEC API Docs"
description: "Guides for SEC API workflows, examples, and evidence-backed investor data."
---

# SEC API Docs

<Info>
Use these guides to solve investor workflows quickly. They keep the path short: start with the job, copy the request or prompt, and keep the evidence metadata with the result.
</Info>
...
## What these docs are designed to do

<Steps>
  <Step title="Show the best path through the API">
    You should not need to memorize the entire API before getting something useful out of SEC API.
  </Step>
...
<CardGroup cols={3}>
  ...
  <Card title="Migrate from sec-api.io" icon="git-compare" href="/migrate-from-sec-api">
    Translate common sec-api.io workflows directly onto SEC API.
  </Card>
