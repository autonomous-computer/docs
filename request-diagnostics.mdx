...
description: "Use request diagnostics to investigate SEC API workflows with support."
---

# Request Diagnostics

SEC API exposes request-scoped diagnostics so you can share the exact workflow context support needs to investigate an issue.
...
```bash
curl "$SEC_API_URL/v1/diagnostics/requests/req_123" \
...
```bash
curl "$SEC_API_URL/v1/diagnostics/deliveries/summary?since=2026-03-14T00:00:00Z&limit=200" \
  -H "Authorization: Bearer $SEC_API_KEY"
```
