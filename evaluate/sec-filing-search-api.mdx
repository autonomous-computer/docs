...
description: "Evaluate SEC filing search for applications and agents: filing manifests, text retrieval, cited sections, and proof-of-concept steps."
---
...
- Section retrieval with `/v1/sections/search` or a filing-section path route
- Metadata preservation: CIK, accession number, filing date, source URL, and
  request metadata where the route supplies it

## Proof-of-concept prompt

<Prompt>
Evaluate SEC API filing search for an investor-agent workflow. Resolve AAPL,
find the latest 10-K, retrieve Item 1A, run semantic search for AI regulation
risk across recent 10-K filings, and return the accession, filing date, source
URL, cited section, degraded state when present, and exact endpoint used.
</Prompt>
...
<Steps>
  <Step title="Start with one issuer">
    Resolve a ticker or CIK, fetch the latest filing with `form=10-K`, and
    confirm the response carries enough identifiers for downstream joins.
  </Step>
  <Step title="Search a real research question">
    Run full-text and semantic search for the same question. Semantic and
    section search can return cited passages; full-text has a different
    filings-and-sections response shape.
  </Step>
