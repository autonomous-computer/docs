---
title: "Hosted MCP Workflows"
description: "Use SEC API from MCP-aware coding agents and clients."
---

# Hosted MCP Workflows

<Info>
Hosted MCP lets coding agents call SEC API directly from MCP-aware tools. Use it when you want agents to resolve issuers, fetch filings, extract sections, and run intelligence workflows without writing a custom integration first.
</Info>

## Why Use Hosted MCP

<CardGroup cols={3}>
  <Card title="Less wrapper code" icon="braces">
    Your agent can call SEC API tools directly from its MCP client.
  </Card>
  <Card title="Same workflows" icon="equal">
    REST and hosted MCP sit on top of the same underlying SEC API workflows.
  </Card>
  <Card title="Faster setup" icon="rocket">
    A working MCP install gets you to a useful issuer, filing, or section workflow quickly.
  </Card>
</CardGroup>

## Install Metadata

```bash
curl https://api.secapi.ai/mcp
```

That payload includes:

- the canonical streamable HTTP MCP transport URL
- auth metadata links
- sponsored bootstrap URLs for human token issuance and machine bootstrap
- install hints
- current tool and resource summaries

<Steps>
  <Step title="Fetch the install metadata">
    Confirm the hosted MCP endpoint is live and returning its transport and auth metadata.
  </Step>
  <Step title="Set the API key">
    Export `SECAPI_API_KEY` or configure the same value as an `x-api-key` header in your MCP client.
  </Step>
  <Step title="Register the server in your client">
    Add the streamable HTTP MCP endpoint to Claude Code, Cursor, or another MCP-aware client.
  </Step>
  <Step title="Exercise a real tool path">
    Resolve an issuer, fetch a filing, and extract a section before using the server in production workflows.
  </Step>
</Steps>

## Common clients

### Claude Code

```bash
claude mcp add --transport streamable-http sec-api https://api.secapi.ai/mcp \
  --header "x-api-key: $SECAPI_API_KEY"
```

### Cursor

```json
{
  "sec-api": {
    "url": "https://api.secapi.ai/mcp",
    "headers": {
      "x-api-key": "${SECAPI_API_KEY}"
    }
  }
}
```

## Good first tools

- `entities.resolve`
- `filings.search`
- `filings.latest`
- `sections.get`
- `statements.get`
- `market.snapshots`
- `macro.search`
- `macro.indicators`
- `macro.high_signal_pack`
- `macro.releases`
- `macro.calendar`
- `macro.forecasts`
- `macro.regimes`
- `macro.credit_ratings`
- `macro.country_report`
- `factors.returns`
- `factors.history`
- `factors.sparklines`
- `factors.extreme_moves`
- `factors.extreme_pairs`
- `factors.valuations`
- `factors.valuation_stocks`
- `portfolio.analyze`
- `portfolio.attribution`
- `portfolio.hedge`
- `intelligence.query`

## AI query quota

MCP `tools/call` requests for `intelligence.query` and `signals.dilution.enhanced.get` count toward the monthly `ai_queries` quota. Successful calls include a top-level `_secapi.quota` envelope with projected post-call usage:

```json
{
  "jsonrpc": "2.0",
  "id": "quota_example",
  "result": {},
  "_secapi": {
    "quota": {
      "family": "ai_queries",
      "mcpToolName": "intelligence.query",
      "limit": 600,
      "used": 42,
      "remaining": 558,
      "resetAt": "2026-05-01T00:00:00.000Z"
    }
  }
}
```

When the quota is exhausted, hosted MCP returns HTTP `429` with JSON-RPC error data such as `code: "ai_query_quota_exceeded"`. Treat quota exhaustion as non-retryable until reset or plan upgrade. If a tool circuit is open, retry only after availability backoff.

<Prompt description="Copy this SEC API prompt for your agent.">
Set up SEC API hosted MCP in this MCP-aware client. Register the streamable HTTP MCP endpoint, resolve AAPL, fetch the latest 10-K, extract Item 1A, and return the result with requestId, provenance, freshness, and materialization metadata intact.
</Prompt>

## Macro Workflow

Use this sequence when an agent needs macro context for a company, factor, or portfolio analysis:

1. Call `macro.search` with a natural-language query such as `inflation` or `policy rate`.
2. Call `macro.high_signal_pack` with `country: "US"` for the compact country pack.
3. Call `macro.calendar` for upcoming scheduled releases and `macro.releases` for released history.
4. Call `macro.forecasts` and `macro.regimes` for forward and backdrop context.
5. Call `macro.country_report` when the agent needs one compact rollup.

```json
{
  "jsonrpc": "2.0",
  "id": "macro_workflow",
  "method": "tools/call",
  "params": {
    "name": "macro.high_signal_pack",
    "arguments": {
      "country": "US",
      "response_mode": "compact"
    }
  }
}
```

## Example tool call

```json
{
  "jsonrpc": "2.0",
  "id": 1,
  "method": "tools/call",
  "params": {
    "name": "intelligence.query",
    "arguments": {
      "query": "Decompose AAPL's last 6M return into factors, identify unusual factor moves, then suggest hedges.",
      "entities": ["AAPL"],
      "lookback": "6m",
      "responseMode": "compact_json_and_md"
    }
  }
}
```
