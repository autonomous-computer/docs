...
description: "Try SEC API SEC data endpoints interactively — no signup required"
---
...
<Info>
These examples use live SEC API endpoints. Create a free account or use an existing API key before running them yourself.
</Info>
...
All playground examples work with the `secapi` CLI:
...
secapi filings search --ticker AAPL --form 10-K --limit 3
secapi insiders list --ticker AAPL
```
