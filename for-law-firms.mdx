---
title: "SEC Data for Legal Teams | SEC API"
...
| `/v1/entities/resolve` | `GET` | Resolve ticker, CIK, or company name to a best-match SEC entity |
...
Combine with entity resolution to find the best-match CIK first:
...
| Skill | What it does |
|-------|-------------|
| `sections.search` | Search filing sections for litigation, risk, and regulatory language |
| `filings.search` | Find filings by company, form type, and date range |
| `entities.resolve` | Resolve ambiguous company references to canonical identifiers |
...
<CardGroup cols={2}>
  <Card title="Starter grant" icon="rocket">
    Start with a 150-call account grant. Enough to evaluate enforcement search, entity resolution, and filing lookups for a single matter.
  </Card>
  <Card title="Pay As You Go" icon="credit-card" href="/plans-and-pricing">
    Self-serve API access with usage-based metering. Scale from one matter to firm-wide compliance monitoring without procurement friction.
  </Card>
</CardGroup>
