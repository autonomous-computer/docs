...
description: "How the SEC API SDKs handle transient failures with safe-by-default retries, exponential backoff, and a client-side circuit breaker."
---

# SDK reliability

<Info>
The JavaScript and Python SDKs include safe-by-default retries, exponential backoff, and a client-side circuit breaker for transient SEC API failures.
</Info>
...
- Always retried regardless of method: `429` rate limits, with `Retry-After` or structured retry timing honored unless retry is disabled
...
Both SDKs also fall back to structured JSON retry timing (`retryAfterSeconds`, `retry_after_seconds`, `retryAfterMs`, or `retry_after_ms`) when a proxy or browser path drops the `Retry-After` header.
