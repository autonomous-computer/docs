...
curl -X POST https://api.secapi.ai/v1/monitors \
  ...
  -d '{
    ...
    "filters": { "form": "8-K" },
    "webhookUrl": "https://your-app.example.com/hooks/secapi"
  }'
```
...
Worst-case latency from filing publish to email arrival is approximately 7.5 minutes (half the 15-min tick interval). Sub-2-second alerting is planned for a future release.
...
Rate-limit math runs primarily through Redis (sliding-window ZSET via the same primitive that backs HTTP middleware). When Redis is unavailable, SEC API falls back to a Postgres count query against `email_delivery_attempts`. When both are unhealthy, the dispatcher fails open with a Sentry warning — better to potentially over-cap than to silently drop alerts.
