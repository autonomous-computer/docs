...
In the signed-in [SEC API dashboard](https://secapi.ai/app/webhooks), register a public HTTPS endpoint subscribed to `monitor.match`, then create the 10-K and 10-Q monitor that should feed it. You can choose email delivery there as well. These organization settings require a WorkOS browser session; API keys cannot create or change them.
