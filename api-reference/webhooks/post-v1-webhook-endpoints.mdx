...
<Prompt>
Use SEC API POST /v1/webhook_endpoints to create a signed webhook endpoint for Delivery events. Preserve `requestId`, `traceparent` in the output. Return a concise markdown summary plus the exact structured payload fields that a downstream engineer or agent should keep using this result.
</Prompt>
