...
description: "Return intraday factor snapshots for dashboards using the current benchmark proxy surface and freshness metadata"
---
  ...
  "data": [
    {
      ...
      "window": "1d",
      "snapshotAt": "2026-06-09T19:45:00.000Z",
      "benchmarkSymbols": [
        "VLUE",
        "IWD",
        "IWF"
...
<Prompt>
Use SEC API GET /v1/factors/returns/intraday to return intraday factor snapshots for dashboards using the current benchmark proxy surface and freshness metadata. Preserve `requestId`, `traceparent` in the output. Return a concise markdown summary plus the exact structured payload fields that a downstream engineer or agent should keep using this result.
</Prompt>
