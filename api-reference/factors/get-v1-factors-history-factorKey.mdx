...
Use SEC API GET /v1/factors/history/{factorKey} to return one factor's daily return series and summary windows from 1D through MAX. Preserve `requestId`, `traceparent`, freshness, and methodology metadata. Prefer `response_mode=compact` when the next step is a chart, table, or model prompt.
</Prompt>
