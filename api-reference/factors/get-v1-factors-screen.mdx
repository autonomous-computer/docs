---
title: "GET /v1/factors/screen"
description: "Return factor screens ranked by recent strength so agents can run factor-rotation and cross-factor ranking workflows"
---
...
curl -X GET \
  ...
  "https://api.secapi.ai/v1/factors/screen"
```
</RequestExample>
...
<Prompt>
Use SEC API GET /v1/factors/screen to return factor screens ranked by recent strength so agents can run factor-rotation and cross-factor ranking workflows. Preserve `requestId`, `traceparent` in the output. Return a concise markdown summary plus the exact structured payload fields that a downstream engineer or agent should keep using this result.
</Prompt>
