...
Use SEC API GET /v1/factors/screen to return factor screens ranked by recent strength so agents can run factor-rotation and cross-factor ranking workflows. Preserve `requestId`, `traceparent` in the output. Keep the response fields your application uses for follow-up requests.
</Prompt>
