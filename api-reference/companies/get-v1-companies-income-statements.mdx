...
description: "Retrieve an issuer's SEC-derived income statement history with normalized operating and per-share fields"
---

Retrieve an issuer's SEC-derived income statement history with normalized operating and per-share fields
...
curl -X GET \
  ...
  "https://api.secapi.ai/v1/companies/income-statements?ticker=AAPL"
```
</RequestExample>
...
<Prompt>
Use SEC API GET /v1/companies/income-statements to retrieve an issuer's SEC-derived income statement history with normalized operating and per-share fields. Preserve `requestId`, `traceparent`, `provenance`, `freshness`, `materialization` in the output. Return a concise markdown summary plus the exact structured payload fields that a downstream engineer or agent should keep using this result.
</Prompt>
