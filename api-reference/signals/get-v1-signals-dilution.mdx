...
Use SEC API GET /v1/signals/dilution to return the SEC API Dilution Score for an issuer. The response is a single 0-100 derived metric with a band (low/moderate/elevated/high), confidence rating, factor breakdown, six categorical sub-ratings (overallRisk, offeringAbilityRisk, dilutionRisk, historicalRisk, cashNeedLevel, warrantExerciseRating), regsho boolean, percentile rank, and provenance. Preserve `requestId`, `traceparent`, `provenance`, `freshness`, `methodologyVersion`, `factors`, `subRatings`, `regsho`, `percentile`, `degradedReasons` in the output. Use `?history=30d|90d|1y` for rolling timeseries reads.
</Prompt>
...
- when `confidence` is `low` or `freshness.status` is `stale`/`degraded`, API the `degradedReasons[]` to the user verbatim so trust is not lost in the handoff
