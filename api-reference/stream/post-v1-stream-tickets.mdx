...
Use SEC API POST /v1/stream/tickets to mint a short-lived signed WebSocket stream ticket for the current principal. Preserve `requestId`, `traceparent` in the output. Return a concise summary and keep the structured fields your application needs for follow-up requests.
</Prompt>
