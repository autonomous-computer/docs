...
Use SEC API POST /v1/stream/tickets to mint a short-lived signed WebSocket stream ticket for the current principal. Preserve `requestId`, `traceparent` in the output. Keep the response fields your application uses for follow-up requests.
</Prompt>
