...
Use SEC API GET /v1/funds/nport/holdings to return the latest sec n-port holdings roster with explicit capability semantics and balance-unit metadata. Preserve `requestId`, `traceparent` in the output. Return a concise summary and preserve the structured fields your application needs for follow-up requests.
</Prompt>
