...
Use SEC API GET /v1/search/semantic to semantic vector search across sec filing section content via voyage ai + pinecone, with hybrid keyword + vector rrf and citation citation fields on every result row. Preserve `requestId`, `traceparent` in the output. Return a concise summary and preserve the structured fields your application needs for follow-up requests.
</Prompt>
