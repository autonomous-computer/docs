...
curl -X GET \
  ...
  "https://api.secapi.ai/v1/traces?ids=trc_example_123"
```
</RequestExample>
...
Use SEC API GET /v1/traces to batch resolve shared trace records by trace identifier across filing-derived and supported non-filing datasets. Preserve `requestId`, `traceparent`, `provenance`, `freshness`, `materialization` in the output. Keep the response fields your application uses for follow-up requests.
</Prompt>
