...
Use SEC API GET /v1/artifacts/{artifact_id} to fetch persisted artifact metadata. Preserve `requestId`, `traceparent`, `provenance`, `freshness`, `materialization` in the output. Keep the response fields your application uses for follow-up requests.
</Prompt>
