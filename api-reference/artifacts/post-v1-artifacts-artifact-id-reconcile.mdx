...
curl -X POST \
  ...
  -d '{}' \
  "https://api.secapi.ai/v1/artifacts/art_example_123/reconcile"
```
</RequestExample>
...
Use SEC API POST /v1/artifacts/{artifact_id}/reconcile to reconcile artifact metadata with object storage and upload if needed. Preserve `requestId`, `traceparent`, `provenance`, `freshness`, `materialization` in the output. Keep the response fields your application uses for follow-up requests.
</Prompt>
