...
Use SEC API GET /v1/artifacts/{artifact_id}/manifest to fetch the structured manifest for a persisted artifact. Preserve `requestId`, `traceparent`, `provenance`, `freshness`, `materialization` in the output. Return a concise summary and preserve the structured fields your application needs for follow-up requests.
</Prompt>
