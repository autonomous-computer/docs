...
curl -X GET \
  ...
  "https://api.secapi.ai/v1/artifacts/art_example_123/manifest"
```
</RequestExample>
...
Use SEC API GET /v1/artifacts/{artifact_id}/manifest to fetch the structured manifest for a persisted artifact. Preserve `requestId`, `traceparent`, `provenance`, `freshness`, `materialization` in the output. Keep the response fields your application uses for follow-up requests.
</Prompt>
