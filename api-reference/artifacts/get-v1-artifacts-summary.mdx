...
Use SEC API GET /v1/artifacts/summary to return artifact lifecycle counts by kind, status, and storage mode. Preserve `requestId`, `traceparent`, `provenance`, `freshness`, `materialization` in the output. Keep the response fields your application uses for follow-up requests.
</Prompt>
