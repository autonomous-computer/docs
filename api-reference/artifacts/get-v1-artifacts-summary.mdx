...
Use SEC API GET /v1/artifacts/summary to return artifact lifecycle counts by kind, status, and storage mode. Preserve `requestId`, `traceparent`, `provenance`, `freshness`, `materialization` in the output. Return a concise summary and keep the structured fields your application needs for follow-up requests.
</Prompt>
