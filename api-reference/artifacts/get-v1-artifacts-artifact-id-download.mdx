...
<Prompt>
Use Omni Datastream GET /v1/artifacts/{artifact_id}/download to download or redirect to the persisted artifact payload. Preserve `requestId`, `traceparent`, `provenance`, `freshness`, `materialization` in the output. Return a concise markdown summary plus the exact structured payload fields that a downstream engineer or agent should keep using this result.
</Prompt>
