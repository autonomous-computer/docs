...
Use SEC API POST /v1/owners/13F/compare to compare the latest two 13F ownership reports for a manager CIK. Preserve `requestId`, `traceparent`, `provenance`, `freshness`, `materialization` in the output. Return a concise summary and keep the structured fields your application needs for follow-up requests.
</Prompt>
