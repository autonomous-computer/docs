...
curl -X GET \
  ...
  "https://api.secapi.ai/v1/market/search?q=apple&limit=10"
```
</RequestExample>
...
Use SEC API GET /v1/market/search to search for stock tickers by company name or keyword. Preserve `requestId`, `traceparent`, `provenance`, `freshness`, `materialization` in the output. Keep the response fields your application uses for follow-up requests.
</Prompt>
