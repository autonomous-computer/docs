---
title: "GET /v1/market/financials"
description: "Financial statements (income, balance sheet, cash flow) for a ticker via SEC API market data"
---

Financial statements (income, balance sheet, cash flow) for a ticker via SEC API market data
...
curl -X GET \
  ...
  "https://api.secapi.ai/v1/market/financials?ticker=AAPL"
```
</RequestExample>
...
<Prompt>
Use SEC API GET /v1/market/financials to financial statements (income, balance sheet, cash flow) for a ticker via SEC API market data. Preserve `requestId`, `traceparent`, `provenance`, `freshness`, `materialization` in the output. Return a concise markdown summary plus the exact structured payload fields that a downstream engineer or agent should keep using this result.
</Prompt>
