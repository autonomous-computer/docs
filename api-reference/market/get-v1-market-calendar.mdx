...
Use SEC API GET /v1/market/calendar to return market calendar sessions for supported exchanges with source coverage metadata. Preserve `requestId`, `traceparent`, `supportedMarkets`, `per-row provenance` in the output. Return a concise summary and keep the structured fields your application needs for follow-up requests.
</Prompt>
