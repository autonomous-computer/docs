...
<Prompt>
Use Omni Datastream GET /v1/market/estimates to return analyst consensus ratings and price-target estimates for a ticker via massive/benzinga. Preserve `requestId`, `traceparent`, `provenance`, `freshness`, `materialization` in the output. Return a concise markdown summary plus the exact structured payload fields that a downstream engineer or agent should keep using this result.
</Prompt>
