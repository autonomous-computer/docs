...
Return analyst consensus ratings and price-target estimates for a ticker with provenance and freshness metadata
...
Use SEC API GET /v1/market/estimates to return analyst consensus ratings and price-target estimates for a ticker. Preserve `requestId`, `traceparent`, `provenance`, `freshness`, `materialization` in the output. Keep the response fields your application uses for follow-up requests.
</Prompt>
