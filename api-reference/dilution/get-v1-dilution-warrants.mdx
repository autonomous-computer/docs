...
Use SEC API `GET /v1/dilution/warrants` to list warrant schedules and assess overhang. Preserve `requestId`, `traceparent`, and the per-row `verification` block. Flag warrants whose `priceProtection` is true (price-reset / ratchet exposure) and report total potential dilutive shares from `underlyingSharesCalculated` when present.
</Prompt>
