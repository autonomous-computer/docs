...
Use SEC API `GET /v1/dilution/share-float-history` to chart shares-outstanding and public-float deltas over time. Preserve `requestId`, `traceparent`, and `verification`. Compute period-over-period dilution as `(sharesOutstanding[t] - sharesOutstanding[t-1]) / sharesOutstanding[t-1]`.
</Prompt>
