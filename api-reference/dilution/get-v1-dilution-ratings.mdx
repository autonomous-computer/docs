...
Use SEC API `GET /v1/dilution/ratings` to rank tickers by dilution risk. Preserve `requestId`, `traceparent`, and `verification`. Show `overallRisk`, `numericScore`, and the sub-factor breakdown so a human reviewer can validate the rating.
</Prompt>
