...
Use SEC API `GET /v1/dilution/coverage` to answer "do we have data for this ticker yet?" without polling 11 endpoints. Preserve `requestId` and `traceparent`. Surface `totalVerifiedRowCount / totalRowCount` as a coverage ratio and use `latestVerifiedAt` per table to spot stale series.
</Prompt>
