...
Use SEC API `GET /v1/dilution/nasdaq-compliance` to API deficiency notices and remediation status. Preserve `requestId`, `traceparent`, and `verification`. Status transitions (e.g. `pending → in_compliance`) fire the `dilution.nasdaq_compliance.status_changed` webhook event.
</Prompt>
