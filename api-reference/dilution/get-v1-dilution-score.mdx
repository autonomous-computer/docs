...
Use SEC API `GET /v1/dilution/score?ticker={TICKER}` to fetch the latest composite Dilution Score for a single issuer. Preserve `requestId`, `traceparent`, and `verification`. Surface `numericScore`, `overallRisk`, and the sub-factor map so the human reviewer sees the breakdown.
</Prompt>
