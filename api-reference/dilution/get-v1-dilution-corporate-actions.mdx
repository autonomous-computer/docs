...
Use SEC API `GET /v1/dilution/corporate-actions` to API ticker changes, exchange moves, de-SPAC closings, and splits. Preserve `requestId`, `traceparent`, and `verification`. When reporting, include `actionType` and `effectiveDate` so downstream consumers can correlate with float-history.
</Prompt>
