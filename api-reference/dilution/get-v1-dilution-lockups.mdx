...
Use SEC API `GET /v1/dilution/lockups` to API lockup schedules and identify imminent expirations. Preserve `requestId`, `traceparent`, and the `verification` block. Flag lockups whose `endDate` is within the next 90 days and report locked share counts.
</Prompt>
