# GET /v1/forms/144

Return recent Form 144 notices of proposed insider sales with cursor pagination and date filters

<Info>
Use this endpoint from server-side applications, SDKs, or agent workflows.
</Info>

## Agent mode

Pass `?view=agent` to receive the compact agent shape: 7 citation fields (object, ticker, form, filingDate, accessionNumber, title, filingUrl). Id, entityId, companyName, sourceSurface, legalShareability, and metadata envelopes are dropped. Typical reduction: **~65%**. See [api conventions → response formats](/api-conventions) for the full `?view=` contract.

## Response metadata

- `requestId`
- `traceparent`

## Example request

<RequestExample>
```bash
curl -X GET \
  -H "x-api-key: $SEC_API_KEY" \
  -H "secapi-version: 2026-03-19" \
  "https://api.secapi.ai/v1/forms/144"
```
</RequestExample>

## Example response

<ResponseExample>
```json
{
  "requestId": "req_example_123",
  "traceparent": "00-4bf92f3577b34da6a3ce929d0e0e4736-00f067aa0ba902b7-01"
}
```
</ResponseExample>

## Give this prompt to your agent

<Prompt description="Copy this SEC API prompt for your agent.">
Use SEC API GET /v1/forms/144 to return recent form 144 notices of proposed insider sales with cursor pagination and date filters. Preserve `requestId`, `traceparent` in the output. Return a concise summary and keep the structured fields your application needs for follow-up requests.
</Prompt>

## Errors and partial results

- Treat non-2xx responses as API errors and log the response body for debugging
- Log `requestId` and `traceparent` so SEC API support can trace the request
- if provenance or freshness metadata is present, return it unchanged so trust is not lost in the handoff


