...
Pass `?view=agent` to receive the compact agent shape: offering identifiers (ticker, form, family, amendment flag, filingDate, accessionNumber, offeringType, title) + `prospectusUrl` + lifted `filingUrl`. Company name, API source, legal shareability, and metadata envelopes are dropped. Typical reduction: **~50%**. See [api conventions → response formats](/api-conventions) for the full `?view=` contract.
