...
Use SEC API GET /v1/offerings to return recent S-1 and 424B prospectus records with cursor pagination and date filters. Preserve `requestId`, `traceparent`, `provenance`, `freshness`, `materialization` in the output. Keep the response fields your application uses for follow-up requests.
</Prompt>
