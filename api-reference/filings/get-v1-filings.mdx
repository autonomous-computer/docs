...
description: "Search filing manifests by issuer, form, date, text, or accession number with historical coverage and offset pagination"
---

Search filing manifests by issuer, form, date, text, or accession number with historical coverage and offset pagination
...
<Prompt>
Use SEC API GET /v1/filings to search filing manifests by issuer, form, date, text, or accession number with historical coverage and offset pagination. Preserve `data`, `hasMore`, `nextCursor`, `requestId` in the output. Return a concise markdown summary plus the exact structured payload fields that a downstream engineer or agent should keep using this result.
</Prompt>
