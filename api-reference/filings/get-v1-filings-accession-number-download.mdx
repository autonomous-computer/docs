...
Use SEC API GET /v1/filings/{accession_number}/download to redirect to the SEC EDGAR source document for a filing by accession number. Preserve `requestId`, `traceparent`, `provenance`, `freshness`, `materialization` in the output. Keep the response fields your application uses for follow-up requests.
</Prompt>
