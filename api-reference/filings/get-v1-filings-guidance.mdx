...
description: "Filing guidance endpoint (currently unavailable)"
---
...
This endpoint currently returns `404 filings_intelligence_not_available`. It is documented here so clients can prepare for the contract, but it is not available as a guidance source. When it becomes available, preserve citations and verify issuer definitions, units, and assumptions in the original disclosure.
...
<Prompt>
GET /v1/filings/guidance is currently unavailable and returns `404 filings_intelligence_not_available`. Do not use it as an active guidance source. When it becomes available, preserve citations and verify issuer definitions, units, and assumptions in the original disclosure.
</Prompt>
