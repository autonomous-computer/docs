...
Use SEC API GET /v1/filings/latest with `ticker`, `symbol`, or `cik` to retrieve the latest filing for an entity and form. Preserve `requestId`, `traceparent`, `provenance`, `freshness`, `materialization` in the output. Keep the response fields your application uses for follow-up requests.
</Prompt>
