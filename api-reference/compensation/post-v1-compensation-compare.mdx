...
Use SEC API POST /v1/compensation/compare to compare the latest two executive compensation disclosures for an issuer. Preserve `requestId`, `traceparent`, `provenance`, `freshness`, `materialization` in the output. Return a concise summary and keep the structured fields your application needs for follow-up requests.
</Prompt>
