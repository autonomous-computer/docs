...
<Prompt>
Use SEC API POST /v1/strategies/factor-rotation to return factor-rotation recommendations informed by macro regime context and factor state. Preserve `requestId`, `traceparent` in the output. Return a concise markdown summary plus the exact structured payload fields that a downstream engineer or agent should keep using this result.
</Prompt>
