...
description: "Deactivate a monitor (idempotent; sets is_active=false)"
---
...
This operation sets `isActive=false` and is idempotent. It does not erase the monitor record or assert that previously emitted events were recalled. Keep the returned monitor state as the audit record of the requested change.
