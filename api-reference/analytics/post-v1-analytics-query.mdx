...
Use SEC API POST /v1/analytics/query to run a tenant-safe analytical query over supported SEC API history datasets without exposing raw SQL. Preserve `requestId`, `traceparent`, `dataset`, `dimensions`, `filters`, `timeWindow`, `sort`, `rowCount`, `warnings`, `provenance` in the output. Return a concise summary and keep the structured fields your application needs for follow-up requests.
</Prompt>
