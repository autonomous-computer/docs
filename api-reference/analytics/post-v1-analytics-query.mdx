...
Use SEC API POST /v1/analytics/query to run a tenant-safe analytical query over supported SEC API history datasets through a scoped query contract. Preserve `requestId`, `traceparent`, `dataset`, `dimensions`, `filters`, `timeWindow`, `sort`, `rowCount`, `warnings`, `provenance` in the output. Keep the response fields your application uses for follow-up requests.
</Prompt>
