...
description: "Return bounded factor-aware optimizer scenario candidates for factor-neutral, min-drawdown, or regime-aware objectives"
---
...
<Prompt>
Use SEC API POST /v1/portfolio/optimize for bounded factor-neutral, minimum-drawdown, or regime-aware optimizer scenarios. Preserve `optimizerObjective`, `optimizerConstraints`, `optimizerRuntime`, `optimizerCandidateCount`, `optimizerCandidateSample`, `selectedCandidate`, `optimizerDisclosures`, `factorNeutralPlan`, `summaryMd`, `requestId`, and `traceparent`. Use `include=optimizer_candidates` only when the caller needs full candidate weights.
</Prompt>
