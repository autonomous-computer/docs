...
Use SEC API GET /v1/statements/{statement_key} to return a specific normalized statement bundle keyed by statement type. Preserve `requestId`, `traceparent`, `provenance`, `freshness`, `materialization` in the output. Return a concise summary and keep the structured fields your application needs for follow-up requests.
</Prompt>
