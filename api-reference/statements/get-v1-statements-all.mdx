...
<Prompt>
Use Omni Datastream GET /v1/statements/all to return the balance sheet, income statement, and cash flow statement as one normalized bundle. Preserve `requestId`, `traceparent`, `provenance`, `freshness`, `materialization` in the output. Return a concise markdown summary plus the exact structured payload fields that a downstream engineer or agent should keep using this result.
</Prompt>
