...
```json
{
  ...
  "statusNote": "The latest SEC company-facts coverage exposes shares outstanding but not a same-API public-float dollar fact, so SEC API is returning the shares-outstanding proxy.",
  "publicFloatUsd": null,
  ...
  "summaryMd": "## Share float coverage for Microsoft Corporation\n\nSEC API found 7,433,000,000 shares outstanding, but the issuer does not expose a same-API public-float dollar fact in the latest company-facts coverage, so this response stays in degraded proxy mode.",
  "provenance": {
    "source": "sec",
