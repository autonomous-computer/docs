...
```bash
export SEC_API_KEY="your-key-here"
```

Don't have a key? [Get an API key](https://secapi.ai/login).
...
```
  ...
  <Tab title="CLI">
```bash
# Requires @secapi/cli when the public package is available.
...
<CardGroup cols={3}>
  <Card title="Install the CLI" icon="terminal" href="/libraries-and-sdks">
    Public package name: `@secapi/cli`
  </Card>
