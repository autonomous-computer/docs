---
title: "Try It"
description: "Interactive API explorer — test SEC API endpoints with your API key"
---
...
```bash
export SECAPI_API_KEY="your-key-here"
```

Don't have a key? Create an account at secapi.ai/signup.
...
Create the saved monitor from the signed-in [dashboard](https://secapi.ai/app/webhooks). Monitor creation is a human-authenticated organization action, so an API key will return `human_auth_required`.

```bash
# Check matches after the monitor is configured.
