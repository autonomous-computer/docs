...
description: "Latency, payload size, and token efficiency comparisons: SEC API vs sec-api.io, Financial Datasets, and edgartools"
---

# Competitive Benchmarks

SEC API is benchmarked against three peers — sec-api.io and Financial Datasets (hosted APIs) and edgartools (open-source Python library) — across core SEC data operations. All benchmarks are reproducible and run from the same network location against production endpoints.

<Info>
Last updated: 2026-03-18 (vs sec-api.io and Financial Datasets); 2026-04-23 preliminary (vs edgartools, pending external methodology review). Methodology and reproduction scripts are in the [benchmarks/](https://github.com/autonomous-computer/sec-api/tree/main/benchmarks) directory; see [`benchmarks/METHODOLOGY.md`](https://github.com/autonomous-computer/sec-api/blob/main/benchmarks/METHODOLOGY.md) for the peer-reviewable methodology doc.
</Info>
...
<Warning>
**Preliminary — pending external methodology review.** Numbers below come from
the reproducible benchmark harness described in
[`benchmarks/METHODOLOGY.md`](https://github.com/autonomous-computer/sec-api/blob/main/benchmarks/METHODOLOGY.md).
</Warning>
...
<Warning>
FinanceBench results are only meaningful with the dated harness context shown
below. Use the current canary result when evaluating SEC API against this
benchmark.
</Warning>
...
| Gate | Target | SEC API canary-25 (2026-04-25) | Methodology |
|---|---|---|---|
| 25-question canary, judge-correct | ≥23/25 (92%) | **18/25 (72%)** | SEC API data-layer tool harness |
| 25-question canary, rule-based | ≥23/25 (92%) | 19/25 (76%) | Same harness; structural API-correctness scoring |

The 72% canary result reflects a dated regression check against production
data. It should not be generalized to the full 150-question corpus or quoted as
an agent-chat benchmark.

## Methodology

- Hosted APIs (SEC API, sec-api.io, Financial Datasets) use production endpoints with authenticated API keys
...
- Full peer-reviewable methodology: [`benchmarks/METHODOLOGY.md`](https://github.com/autonomous-computer/sec-api/blob/main/benchmarks/METHODOLOGY.md)

## Reproduce

```bash
# Run sec-api.io benchmark (paid API key)
SECAPI_API_KEY=your_key bun run bench:sec-api

# Run Financial Datasets benchmark (paid API key)
