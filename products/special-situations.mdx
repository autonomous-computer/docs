---
title: "Special Situations API"
description: "Track durable special situations — M&A, tender offers, going-private, spin-offs, activist campaigns, restructuring, and bankruptcy — with lifecycle status, deal terms, per-filing timelines, and browse-by-form access through one SEC API surface."
---

Track durable special situations across their full lifecycle. Mergers, tender offers, going-private transactions, spin-offs, activist campaigns, restructuring, and bankruptcy — each with lifecycle status, deal terms, a market snapshot, and a per-filing timeline assembled from the underlying EDGAR record.

<Info>
The public [Situations database](https://secapi.ai/situations) is a free, recent, rate-limited research and discovery surface. This API family is the paid, authenticated product: it unlocks the complete archive, higher limits, source-cited Copy-for-LLM exports, and application or agent workflows.
</Info>

## Why use this

A special situation is not a single filing — it is a durable, evolving object that accretes filings over weeks or months. The SEC API Special Situations surface stitches the triggering and follow-on filings into one situation with a stable `situation_id`, a lifecycle status, and a timeline, so agents and applications never have to reassemble a deal from raw EDGAR events.

- **Durable situations** -- one object per deal, with a stable `situation_id` that survives across follow-on filings
- **Lifecycle status** -- announced, pending, completed, terminated, and related states, so deal risk is legible at a glance
- **Browse by EDGAR form** -- resolve "what did this `SC 13D` / `SC TO-T` / `425` / `DEFM14A` filing trigger?" directly, with the form expanded to the situation types it drives
- **Per-filing timeline** -- the ordered filing history behind each situation, available inline or as a paginated sub-resource
- **Enriched or stripped** -- request the full projection, or the lean stripped shape for wide scans (billed identically)
- **Agent-ready evidence** -- carry issuer identifiers, source filings, freshness metadata, field availability, and machine-readable lifecycle fields through the same workflow

## Quick start

```bash
# List active mergers
curl -H "x-api-key: $SECAPI_API_KEY" \
  "https://api.secapi.ai/v1/situations?types=merger&limit=10"
```

## Enriched vs. stripped

Every situation list and detail endpoint accepts `enrich`:

- `enrich=true` (default) returns the full projection — deal terms, market snapshot, and timeline context.
- `enrich=false` returns the minimal stripped projection: identity, status, and key dates only.

Both are **billed identically** — choose `enrich=false` only to reduce payload size on wide scans, never to save quota.

```bash
# Lean stripped shape for a wide scan
curl -H "x-api-key: $SECAPI_API_KEY" \
  "https://api.secapi.ai/v1/situations?statuses=pending&enrich=false&limit=100"
```

## Browse by form

When you already have an EDGAR form in hand and want the situations it opened or advanced, use the by-form endpoint. The form is expanded server-side to the situation types it triggers. URL-encode the form (a space becomes `%20`):

```bash
curl -H "x-api-key: $SECAPI_API_KEY" \
  "https://api.secapi.ai/v1/situations/by-form/SC%2013D?limit=10"
```

Filter further with `statuses`, `tickers`, and `enrich`. Common triggering forms include `SC 13D` (activist stake), `SC TO-T` (third-party tender offer), `425` (merger communications), and `DEFM14A` (merger proxy).

## Weekly issue archive

The free [Situations issue archive](https://secapi.ai/situations/issues) is an organic discovery surface: each numbered edition is a frozen public snapshot of the SEC-derived situations published for that week. It is designed for research, sharing, and search discovery, with reasonable anonymous limits.

Use the authenticated archive API when an application or agent needs the complete paid archive, higher limits, and durable machine-readable issue snapshots. An issue never mutates after publication, even when a source situation later advances, closes, or is corrected.

```bash
# Browse numbered editions
curl -H "x-api-key: $SECAPI_API_KEY" \
  "https://api.secapi.ai/v1/situations/issues?limit=12"

# Retrieve one frozen edition by issue number or slug
curl -H "x-api-key: $SECAPI_API_KEY" \
  "https://api.secapi.ai/v1/situations/issues/22"
```

Archive reads use the `situations_lookup` meter class. The standard API plan and account limits determine availability; inspect your current allowance through the billing and limits endpoints rather than relying on a hard-coded quota in documentation.

## The filing timeline

Each situation carries an ordered, oldest-first timeline of the filings that built it. Fetch it inline with the situation, or page it as a standalone sub-resource:

```bash
# Full situation, timeline included
curl -H "x-api-key: $SECAPI_API_KEY" \
  "https://api.secapi.ai/v1/situations/sit_abc123"

# Just the timeline, paginated
curl -H "x-api-key: $SECAPI_API_KEY" \
  "https://api.secapi.ai/v1/situations/sit_abc123/filings?limit=50"
```

## Compact summary

When you need the headline — rendered markdown, deal terms, and the latest timeline event — without the full timeline, use the summary sub-resource:

```bash
curl -H "x-api-key: $SECAPI_API_KEY" \
  "https://api.secapi.ai/v1/situations/sit_abc123/summary"
```

## Copy for an LLM, without losing provenance

Use the Markdown export when an agent needs a concise underwriting-ready brief rather than a wide JSON object. It preserves source filing links, accession numbers, dates, and explicitly absent fields. Treat it as evidence to inspect, never as an investment recommendation.

```bash
curl -H "x-api-key: $SECAPI_API_KEY" \
  "https://api.secapi.ai/v1/situations/sit_abc123/export"
```

For machine-readable timeline and field-level work, retrieve the detail or filings resource instead.

## Underwriting pack

When an underwriting workflow needs one deterministic evidence bundle, use the underwriting pack endpoint. It returns public canonical situation detail, the SEC filing timeline, source citations, and Copy-for-LLM markdown as one authenticated paid heavy extract. It has no query options; the only input is the `situation_id`.

```bash
curl -H "x-api-key: $SECAPI_API_KEY" \
  "https://api.secapi.ai/v1/situations/sit_abc123/underwriting-pack"
```

The pack is public-data only: no internal research systems, TIKR data, private analyst notes, recommendations, or investment advice are included. Until the endpoint is available in your environment, assemble the same public evidence from the detail, filings, and export endpoints.

## Monitor a situation

Create durable Situation monitors through the authenticated Monitors API. A watch is a monitor with `searchMode: "situation"`; it uses the same retry, delivery ledger, webhook, stream, and billing controls as the rest of the delivery platform.

- **Broad watches** filter by type, subtype, status, ticker, or sector. They begin at database time by default, so creating a watch does not replay the historical archive. Their `monitor.match` events contain a lifecycle snapshot and `situationId`; fetch situation detail when you need the filing timeline.
- **Exact watches** use `filters.situationIds`. Their deliveries additionally include filing-level provenance, including the source accession and source URL for new timeline events.
- Configure an organization webhook endpoint or stream subscription for `monitor.match`. Do not use the legacy per-monitor `webhookUrl` field for new integrations.

```bash
curl -X POST \
  -H "x-api-key: $SECAPI_API_KEY" \
  -H "content-type: application/json" \
  -d '{"name":"Acquisition watch","query":"situations.watch","searchMode":"situation","filters":{"types":["merger","tender_offer"],"tickers":["AAPL","MSFT"]}}' \
  "https://api.secapi.ai/v1/monitors"
```

Actual webhook, stream, and email deliveries use their respective delivery meters (`webhook_delivery`, `stream_event`, and `email_delivery`). Monitor creation is a control-plane action; inspect your plan and delivery allowances through the billing and limits endpoints.

## Endpoints

| Endpoint | Description |
|----------|-------------|
| `GET /v1/situations` | List situations; filter by `types`, `statuses`, `tickers`, `forms`, `enrich` |
| `GET /v1/situations/issues` | List immutable numbered weekly archive issues |
| `GET /v1/situations/issues/{issue}` | Retrieve a frozen archive issue by number or slug |
| `GET /v1/situations/feed` | Reverse-chronological feed of situation events |
| `GET /v1/situations/calendar` | Upcoming key dates (record, vote, expiry, expected close) |
| `GET /v1/situations/stats` | Counts by type, status, sector, and market-cap bucket |
| `GET /v1/situations/performance` | Closed-situation outcome cohorts |
| `GET /v1/situations/by-form/{form}` | Situations opened or advanced by an EDGAR form |
| `GET /v1/situations/{situation_id}` | One situation with its full per-filing timeline |
| `GET /v1/situations/{situation_id}/filings` | Paginated per-filing timeline |
| `GET /v1/situations/{situation_id}/summary` | Compact summary: markdown, deal terms, latest event |
| `GET /v1/situations/{situation_id}/export` | Source-cited Markdown Copy-for-LLM brief |
| `GET /v1/situations/{situation_id}/underwriting-pack` | Source-cited paid heavy extract for underwriting workflows |
| `POST /v1/monitors` | Create a typed `searchMode: "situation"` Special Situations watch |

## SDK and CLI

The `secapi` CLI mirrors the plane:

```bash
secapi situations list --types merger --limit 10
secapi situations issues --limit 12
secapi situations issue --issue 22
secapi situations by-form --form "SC 13D"
secapi situations get --situation-id sit_abc123
secapi situations filings --situation-id sit_abc123
secapi situations summary --situation-id sit_abc123
secapi situations watch --tickers AAPL,MSFT --types merger,tender_offer
```

The JavaScript SDK exposes the same surface under `client.situations`:

```js
await client.situations.list({ types: "merger", limit: 10 })
await client.situations.issues({ limit: 12 })
await client.situations.issue("22")
await client.situations.byForm("SC 13D")
await client.situations.get("sit_abc123")
await client.situations.filings("sit_abc123")
await client.situations.summary("sit_abc123")
await client.createMonitor({
  name: "Acquisition watch",
  query: "situations.watch",
  searchMode: "situation",
  filters: { types: ["merger", "tender_offer"], tickers: ["AAPL", "MSFT"] },
})
```

MCP clients can use `situations.issues` to browse the archive and `situations.issue` to retrieve one frozen issue. Both are authenticated paid tools using the `situations_lookup` meter class. Monitor creation is intentionally a mutating API/CLI workflow today; configure an agent with explicit mutation authority before creating watches.

## Product boundary

This documentation describes the customer-facing Special Situations plane only. Internal research systems outside this product are not part of the public API, SDK, CLI, MCP, pricing, or marketing surface.

## Related

- [API Reference: Special Situations](/api-reference/situations)
- [Filing Search API](/products/filing-search)
- [Ownership & Insider Trading API](/products/ownership)
